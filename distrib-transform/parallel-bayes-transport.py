...
embedders are bypassed completely: particles enter the Posterior Transformer in physical S*D width and
observations enter the causal likelihood Transformer in physical D+1 width, with zero learnable
parameters in either fixed-shape input embedder.  Training uses iid simulator rows together with a synthetic
...
1. Sample the active problem shape S,D.  Let rho_0 be ONE configured base prior: either the uniform
   ...
   N(0, prior_std^2 I).  Draw N iid full-theta particles z_1,...,z_N ~ rho_0, one synthetic potential
   truth theta_tilde ~ rho_0, and an interpolation time tau ~ Uniform[0,1].
2. Form the synthetic input prior cloud by the flow-matching-style interpolation

       c_n = (1-tau) z_n + tau theta_tilde,     n=1,...,N.

   Thus tau=0 is the ordinary base-prior cloud and tau near 1 is a concentrated cloud near
   theta_tilde.
3. Draw b ~ Bernoulli(0.5).  If b=1 set the actual observation-generating parameter
   theta* = theta_tilde; otherwise draw a fresh independent theta* ~ rho_0.  Only this actual theta*
   belongs to the iid simulator datum.  The potential theta_tilde is synthetic and is used only to
   construct the input cloud.  The b=1 branch mimics posterior-like clouds encountered during
   sequential unrolling; b=0 deliberately trains robustness to a concentrated but misspecified prior.
4. Draw Omax designs and noisy observations Y from the physical likelihood conditional on theta*.
   Rows are independent and no theta* is reused across training rows.  The resulting (theta*,Y)
   pairs retain the ordinary iid base-prior simulator law; the interpolated cloud is an auxiliary
   conditioning input produced by the dataloader.
5. Encode all Omax observations once with the causal likelihood Transformer.  For every prefix
   o=Omin,...,Omax, apply the SAME Posterior Transformer directly to the SAME synthetic input cloud.
   There is no posterior recurrence in the training graph.
...
compatible with this score.  Because the synthetic input cloud is intentionally correlated with
theta* in the matched Bernoulli branch, it should be viewed as an additional random conditioning
variable rather than as an independent reference draw from a separately sampled prior law.

Observation prefixes
--------------------
Each item contains Omax observations.  A causal LikelihoodSequenceEmbedder processes all Omax
observation tokens once and returns Omax contextual tokens.  In fixed-shape mode those tokens are the
parameter-free normalized physical (design,outcome) vectors; otherwise they come from the TAMO-style
observation embedder.  Training NEVER samples one random
...
exactly S*D and the observation/likelihood-token width is exactly D+1.  The fixed-shape input preprocessors
contain zero learnable parameters.  In both regimes the output head returns compact physical theta; only the
...
E : shared embedding width ONLY when S or D varies
...
observation_contexts    [B,Omax,D+1] in fixed-shape mode, otherwise [B,Omax,E]
...
class BayesTransportConfig:
    ...
    # The interpolation time used to make synthetic training priors is ALWAYS Uniform[0,1].
    base_prior_distribution: str = "uniform"  # {"uniform", "gaussian"}
    prior_std: float = 1.0
    synthetic_prior_match_probability: float = 0.5
    ...
    # S*D and observation/likelihood tokens have width D+1, so `embedding_dim` is ignored.
    # When shapes vary, every observation pair and theta particle is mapped to one E-vector.
...
def sample_interpolated_training_prior_and_truth_np(
    ...
    *,
    ...
    """Sample one synthetic training prior cloud and the actual observation truth.

    The CPU sampling order is exactly the experiment described at the top of this file:
      1. z_n iid ~ rho_0, theta_tilde ~ rho_0, tau ~ Uniform[0,1];
      2. C_tau = {(1-tau) z_n + tau theta_tilde}_n;
      3. with probability p use theta*=theta_tilde, otherwise redraw theta* ~ rho_0.

    theta_tilde and tau are deliberately kept local to the dataloader and are not part of
    the simulator datum returned to JAX.
    """
    S = int(num_sources)
    D = int(source_dim)
    ...
    prior_particles = (
        (np.float32(1.0) - tau) * base_particles
        + tau * potential_theta[None, :, :]
    ).astype(np.float32)
    ...
    return prior_particles, theta_true.astype(np.float32)
...
def sample_interpolated_prior_given_truth_np(
    ...
    *,
    ...
    """Sample C_tau conditional on an already-generated iid theta* for validation.

    This is the conditional form of the same 50/50 training law.  With probability p the
    synthetic potential theta is the supplied theta*; otherwise it is an independent base-
    prior draw.  Fresh z_n and tau ~ Uniform[0,1] are then used to build the cloud.  Using a
    fixed NumPy seed in validation therefore reproduces the same clouds every epoch.
    """
    S = int(num_sources)
    D = int(source_dim)
    base_particles = sample_base_prior_np(
...
def simulate_iid_joint_samples(
    ...
    *,
    ...
    """Generate iid simulator pairs (theta*,Y) from the ONE configured base prior.

    We reuse the trusted trajectory simulator with trajectory_length=1 and remove that
    singleton axis.  Each row independently draws theta* ~ rho_0, Omax designs, and
    observation noise.  No synthetic potential theta or interpolation time is stored here;
    make_iid_batch_np supplies the auxiliary interpolated cloud when evaluating the direct
    training objective.

        (theta_i^*, Y_i) iid ~ gamma_0,
        Y_i = {(x_{i,o}, y_{i,o})}_{o=1,...,Omax}.

    The Omax observations are multiple conditioning prefixes of the SAME iid joint datum,
    not recurrent posterior states.
    """
...
def make_iid_batch_np(
    ...
    *,
    ...
    """Attach one synthetic interpolated input cloud to each iid simulator datum.

    The dataset contains only the actual iid (theta*,Y) rows.  Conditional on each stored
    theta*, this helper samples exactly the same auxiliary cloud law as the training
    dataloader: with probability p the synthetic potential theta equals theta*, otherwise
    it is an independent rho_0 draw; in either case fresh z_n ~ rho_0 and
    tau ~ Uniform[0,1] define c_n=(1-tau)z_n+tau*theta_tilde.

    This conditional construction makes fixed-seed validation reproduce the same joint
    training distribution without storing synthetic potential truths in the dataset.
    """
    ...
    for b, (dataset_index, S_value, theta_size_value) in enumerate(
        ...
        D = theta_size_int // S
        theta_active = np.asarray(dataset["theta_true"][int(dataset_index)], dtype=np.float32)[:S, :D]
...
class ContinuousJointDataset(IterableDataset):
    ...
    the synthetic potential theta and interpolation time are sampled and consumed locally;
...
class LikelihoodSequenceEmbedder(eqx.Module):
    """Causally contextualise all Omax observation tokens exactly once.
    ...
    In heterogeneous-shape mode the input/output width is the shared embedding dimension E.
    In fixed-shape mode it is the physical observation width D+1, so the likelihood Transformer
    itself operates directly on normalized (design,outcome) vectors with no learned input embedder.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.token_dim = cfg.embedding_dim if token_dim is None else int(token_dim)
        if self.token_dim < 1:
            raise ValueError("Likelihood token dimension must be positive.")
        # Direct physical widths such as D+1=3 need not be divisible by the heterogeneous
        # embedding head count.  Use the largest head count shared by both numbers; for the
        # common D=2 case this correctly gives one head without introducing a projection.
        self.attention_heads = max(1, math.gcd(self.token_dim, int(cfg.embedding_heads)))
        keys = jax.random.split(key, cfg.dimension_embedder_depth + 1)
        self.blocks = tuple(
            CausalObservationBlock(
                self.token_dim,
                self.attention_heads,
                cfg.mlp_ratio * self.token_dim,
                key=keys[i],
            )
            for i in range(cfg.dimension_embedder_depth)
        )
        self.final_norm = eqx.nn.LayerNorm(self.token_dim)
...
class SequentialBayesModel(eqx.Module):
    ...
    def __init__(self, cfg: BayesTransportConfig, *, key: Array):
        ...
        if self.fixed_shape:
            ...
            # Posterior Transformer at width S*D, while likelihood tokens stay at width D+1.
            self.particle_input_dim = cfg.max_num_sources * cfg.max_source_dim
            self.observation_context_dim = cfg.max_source_dim + 1
            ...
            self.observation_context_dim = cfg.embedding_dim
        ...
        self.likelihood_embedder = LikelihoodSequenceEmbedder(
            cfg,
            key=likelihood_key,
            token_dim=self.observation_context_dim,
        )
    ...
    def _encode_observation_block(
        ...
        return self.likelihood_embedder(pair_embeddings)  # [Omax,D+1] fixed; [Omax,E] heterogeneous
    ...
    def predict_prefixes(
        ...
        All prefix transports share the exact same supplied input cloud.  The causal
    ...
    def __call__(
        ...
        )(observations)  # [T,Omax,D+1] fixed; [T,Omax,E] heterogeneous
...
def print_model_parameter_count(model: SequentialBayesModel):
    ...
    if model.fixed_shape:
        print(
            "  Fixed-shape direct widths: "
            f"theta={model.particle_input_dim}, observation={model.observation_context_dim}"
        )
...
def batch_objective(
    ...
    Every batch row contains one actual iid simulator pair (theta*,Y) from the fixed base-prior
    data law plus ONE synthetic interpolated input cloud C_tau generated by the CPU dataloader.
    The cloud is intentionally correlated with theta* in the matched Bernoulli branch and is
    independent of theta* in the misspecified branch.  The causal observation encoder is
    evaluated once, and the direct Posterior Transformer is vmapped over every
    ...
    because C_tau deliberately contains information about theta* with probability p.
...
def plot_architecture_schematic(
    ...
    draw_box(
        (0.02, 0.47), 0.14, 0.14,
        f"z_n, theta_tilde ~ rho_0\ntau ~ U[0,1]; p={cfg.synthetic_prior_match_probability:.2f} match\n"
        "C_tau=(1-tau)z+tau theta_tilde",
...
def train_model(
    ...
    Every training row contains one independent actual (theta*,Y) simulator draw together
    with one CPU-generated interpolated input cloud C_tau from the experiment described at
    the top of this file.  One causal observation pass yields Omax contextual tokens, then
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        save_json(
            ...
            {
                "training": (
                    "iid base-prior simulator pairs plus one synthetic interpolated input cloud; "
                    "direct transports for all observation prefixes"
                ),
                "data_assumption": (
                    "actual (theta*,Y) rows are iid from the single configured base-prior simulator; "
                    "C_tau is an auxiliary dataloader-generated conditioning variable"
                ),
                ...
                "interpolation_time_distribution": "Uniform[0,1]",
...
if train_wm:
    ...
    print("  synthetic prior interpolation time: tau ~ Uniform[0,1]")
    print(f"  P(theta*=theta_tilde): {CFG.synthetic_prior_match_probability:.3f}")
...
summary = {
    ...
    "data_assumption": (
        "actual (theta*,Y) rows are iid from one fixed base-prior simulator; "
        "the interpolated cloud is an auxiliary dataloader-generated conditioning variable"
    ),
    ...
    "synthetic_prior_interpolation": "C_tau=(1-tau)Z+tau*theta_tilde",
    "interpolation_time_distribution": "Uniform[0,1]",
    "synthetic_prior_match_probability": CFG.synthetic_prior_match_probability,
    ...
    "fixed_shape_observation_token_dim": (
        model.observation_context_dim if FIXED_SINGLE_SHAPE_SETUP else None
    ),
    "max_theta_size": CFG.max_num_sources * CFG.max_source_dim,
