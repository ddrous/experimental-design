...
#%% 18) Create the run, heterogeneous datasets, and fixed visualisation trajectory
...
#%% 19) Train the sequential AdaLN model on the heterogeneous Mode-A problem
...
#%% 19b) Train lightweight fixed-dimensional visualisation decoders AFTER main training
...
def train_visualization_decoder(
    ...
    for epoch in range(1, cfg.decoder_epochs + 1):
        ...
        if epoch == 1 or epoch % 10 == 0 or epoch == cfg.decoder_epochs:
            print(f"[sequential] visual decoder epoch {epoch:03d}: MSE={epoch_mse:.6f}")

    save_visualization_decoder(run_dir / "artefacts" / "visualization_decoder.eqx", decoder)
    np.savez_compressed(
        run_dir / "artefacts" / "visualization_decoder_history.npz",
    ...
    ax.grid(alpha=0.25)
    fig.savefig(run_dir / "plots" / "visualization_decoder_training.png", dpi=170)
...
plot_posterior_evolution(
    ...
    "sequential AdaLN: decoded posterior evolution after post-hoc decoder training",
)
