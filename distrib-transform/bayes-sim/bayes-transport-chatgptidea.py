...
"""Single-cloud particle transport trained with an empirical energy-score objective.

This notebook-style file preserves the original source-localisation simulator, optional
dimension-agnostic theta/observation embedders, physical particle representation,
heterogeneous (S,D) support, and sequential evaluation diagnostics. It additionally contains
the stronger training path motivated by the Gaussian-conjugate control experiment.

The main difficulty exposed by source localisation is not the energy score itself: one
observation creates a nonlinear, weakly identified posterior over a joint multi-source
configuration, and repeated Bayes quickly produces prior clouds whose geometry is unlike
simple Euclidean interpolation clouds. The stronger default therefore trains an actual Bayes
operator on a finite empirical prior and exposes it to short self-generated rollouts.

Training prior construction
---------------------------
For each fresh iid training row:

1. Sample the active problem shape S,D. Let rho_0 be ONE configured base prior: either the
   uniform distribution on the physical design box or the isotropic Gaussian
   N(0, prior_std^2 I). Draw N iid full-theta particles z_n ~ rho_0 and one synthetic
   interpolation anchor theta_tilde ~ rho_0.

2. Canonicalize exchangeable sources BEFORE interpolation when source canonicalization is
   enabled. This prevents arbitrary source-label pairings from being interpolated and then
   discontinuously reordered later.

3. Draw interpolation coefficients according to `interpolation_tau_mode` and form

       c_n = (1-tau_n) z_n + tau_n theta_tilde.

   All previous modes remain available: shared_uniform, shared_logit_normal, shared_beta,
   iid_uniform, and hierarchical_beta. The default hierarchical_beta mode combines row-to-row
   concentration variation with particle-to-particle heterogeneity.

Training target: exact empirical Bayes by default
-------------------------------------------------
The default truth is an independent draw from the same interpolated POPULATION law as the
particles. The realised cloud is simultaneously treated as the finite empirical approximation

       p_C(theta) = N^{-1} sum_n delta_{c_n}(theta).

Observations are simulated from the ORIGINAL physical source-localisation likelihood
conditional on theta*. The likelihood is never used to construct the prior cloud. Given C,
the exact Bayes update of this finite empirical approximation is available without approximation:

       w_n^(o) proportional to prod_{j=1}^o p(y_j | x_j, c_n),

       p_C(theta | D_1:o) = sum_n w_n^(o) delta_{c_n}(theta).

The default training objective uses these weights to evaluate the CONDITIONAL EXPECTATION of
the same empirical energy score optimized by the original code. For a learned equally
weighted output cloud Q={q_i},

       E_{Theta ~ p_C(.|D)} ES(Q,Theta)
         = N^{-1} sum_i sum_n w_n ||q_i-c_n||
           - (2N^2)^{-1} sum_{i,j} ||q_i-q_j||.

This is not a new scoring rule: it is the exact conditional expected empirical energy score
for the supplied finite-cloud approximation. It removes most target Monte-Carlo noise and gives
the conditioning observation a direct, row-specific signal. A smaller stochastic energy score
against the independently sampled population-law theta* is added by default so the transport
can learn useful off-support movement when the finite cloud misses the continuous truth. The
old pure stochastic objective remains available with `training_objective_mode="stochastic_energy"`.

Short self-rollout training
---------------------------
The Gaussian conjugate control remained easy because its posterior family is closed under
Bayes. Source localisation is not. To reduce train/evaluation distribution shift, each
training row additionally contains a short sequence of fresh observation blocks generated
from the same theta*. Starting from C, the learned map is applied recurrently for
`training_rollout_length` steps.

Between rollout steps the carried particle cloud is stop-gradient by default. Consequently
there is NO long-horizon backpropagation through time. At every rollout step, the detached
cloud actually entering that update is treated as the finite empirical prior and the physical
likelihood is evaluated on those particles to obtain the exact LOCAL one-step posterior
weights. Thus each recurrent training target is solvable from exactly the information supplied
to that step, even early in training. This is DAgger-like exposure without unstable credit
assignment.

Architecture initialization
---------------------------
The historical combination of exactly-zero AdaLN modulation and an exactly-zero output head
created an identity bootstrap trap: initially only the final head could learn, and it could
not see the observation. The stronger default initializes both paths to SMALL NONZERO scales.
The map is still close to identity, but observation gradients exist on train step 1.

For the common Omin=Omax=1 case, the stronger default also replaces the historical
zero-parameter observation-context bypass with a small nonlinear MLP. The old identity bypass
remains configurable. This gives the inverse-distance likelihood enough learned feature
capacity to represent nonlinear interactions between sensor location and reading.

For a uniform base prior the output head also clips active physical coordinates to the
physical design box. A true Bayesian posterior under that prior has zero mass outside the
box, so allowing the learned transport to leave it only wastes capacity.

Sequential evaluation
---------------------
The ordinary repeated-Bayes stress test is retained. It starts from a fresh base-prior cloud
and repeatedly applies the learned update to fresh observation blocks. Physical design
locations already seen are shown as light-green x markers. Likelihood-based SNIS references
remain diagnostics only; they may increase proposal count adaptively when effective sample
size is too small.

Array shapes
------------
B : iid training rows
N : particles per cloud
P = Omax-Omin+1 : direct observation prefixes
R : training_rollout_length
S,D : active source count and coordinate dimension
Kmax = Smax*Dmax

theta_true                  [B,Smax,Dmax]
observations                [B,Omax,Dmax+1]          # direct first update
prior_particles             [B,N,Smax,Dmax]
posterior_weights_by_prefix [B,P,N]                  # exact empirical Bayes targets
rollout_observations        [B,R,Omax,Dmax+1]
posterior_by_prefix         [B,P,N,Kmax]
training_rollout            [B,R,N,Kmax]
"""
...
class BayesTransportConfig:
    ...
    interpolation_tau_mode: str = "hierarchical_beta"
    ...
    #   "closed_form"      : DEFAULT; independent draw from the same interpolated population law.
    #   "empirical"        : exact draw from the realised N-particle empirical prior (retained ablation).
    #   "bernoulli_choice" : old experiment; theta*=theta_tilde with probability p, otherwise rho_0.
    synthetic_truth_sampling_mode: str = "closed_form"  # {"closed_form", "empirical", "bernoulli_choice"}
    # Used ONLY when synthetic_truth_sampling_mode="bernoulli_choice".
    synthetic_prior_match_probability: float = 1.0

    # Training objective. "empirical_expected_energy" is available when the realised finite
    # cloud is the literal empirical prior; it computes the exact conditional expectation of
    # the original energy score under likelihood-reweighted posterior weights.
    training_objective_mode: str = "empirical_expected_energy"  # {"empirical_expected_energy", "stochastic_energy"}
    # In expected-energy mode, retain a smaller stochastic ES term against the independent
    # population-law theta*. This encourages useful off-support movement when N=64 misses the
    # continuous truth, while the finite-cloud teacher supplies the low-variance Bayes signal.
    stochastic_truth_aux_weight: float = 0.25

    # Short stop-gradient self-rollouts expose the map to posterior-shaped clouds like those
    # encountered in sequential evaluation. Step 1 uses the same observation block as the
    # direct objective; later steps use fresh blocks generated from the same theta*.
    training_rollout_length: int = 4
    training_rollout_observations_per_step: int = 1
    training_rollout_loss_weight: float = 1.0
    training_rollout_stop_gradient_between_steps: bool = True

    # Near-identity but NONZERO initialization; zero biases are retained.
    adaln_modulation_init_scale: float = 0.02
    output_head_init_scale: float = 0.02

    # Uniform-prior posteriors have no support outside the physical design box.
    enforce_uniform_prior_support: bool = True
    ...
    # Observation-prefix and particle counts. Every direct iid training block contains Omax
    # observations and every prefix o=Omin,...,Omax is optimized. Short training self-rollouts
    # additionally consume a fixed configured prefix from each fresh rollout block.
    min_observations_per_step: int = 1
    max_observations_per_step: int = 1
    test_observations_per_step: int = 1
    num_particles: int = 64*4
    n_train_trajectories: int = 4096
    n_eval_trajectories: int = 256
    batch_size: int = 16*8

    # Long held-out sequential-evaluation horizon. Shorter recurrent exposure is separately
    # controlled by training_rollout_length above.
    ...
    # Continuous host-side iid simulator stream. n_train_trajectories remains the number of
    # FRESH iid joint rows consumed per nominal epoch; each row also contains fresh short-rollout
    # observation blocks and is never revisited.
    train_dataloader_num_workers: int = 0
    train_dataloader_prefetch_factor: int = 2

    # Observation-context encoder. For genuine multi-observation prefixes this is the original
    # causal likelihood Transformer. For Omin=Omax=1 there is no temporal sequence; nevertheless
    # source localisation benefits from a nonlinear learned representation of (design,reading).
    # "identity" exactly reproduces the historical zero-parameter single-observation bypass.
    # "mlp" (default) uses two learned projections with SiLU before posterior conditioning.
    single_observation_context_mode: str = "mlp"  # {"identity", "mlp"}
    likelihood_hidden_dim: int = 192
    likelihood_heads: int = 8
    likelihood_mlp_ratio: int = 4
    likelihood_depth: int = 4

    # Posterior Transformer. Direct training vmaps prior->posterior over every prefix; short
    # stop-gradient self-rollouts reuse the same map on its own posterior-shaped clouds.
    ...
    # Optimisation. In the recommended empirical mode, the direct and self-rollout losses are
    # exact conditional expectations of the original empirical energy score under finite-cloud
    # Bayesian posterior weights. The whole interacting cloud remains the reported predictive
    # distribution, so particle self-attention is compatible with the score.
    epochs: int = 2000
    ...
    # Reference-posterior diagnostic only; never enters the training loss. Proposal count
    # grows adaptively if the base-prior SNIS effective sample size is too small.
    reference_proposals: int = 20_000
    reference_max_proposals: int = 500_000
    reference_min_ess: float = 200.0
...
def validate_config(cfg: BayesTransportConfig):
    ...
    if cfg.single_observation_context_mode not in {"identity", "mlp"}:
        raise ValueError("single_observation_context_mode must be 'identity' or 'mlp'.")
    ...
    if cfg.training_objective_mode not in {"empirical_expected_energy", "stochastic_energy"}:
        raise ValueError(
            "training_objective_mode must be 'empirical_expected_energy' or 'stochastic_energy'."
        )
    if cfg.training_rollout_length < 1:
        raise ValueError("training_rollout_length must be >= 1.")
    if not (
        cfg.min_observations_per_step
        <= cfg.training_rollout_observations_per_step
        <= cfg.max_observations_per_step
    ):
        raise ValueError(
            "training_rollout_observations_per_step must lie in "
            "[min_observations_per_step,max_observations_per_step]."
        )
    if cfg.training_rollout_loss_weight < 0.0:
        raise ValueError("training_rollout_loss_weight must be >= 0.")
    if cfg.stochastic_truth_aux_weight < 0.0:
        raise ValueError("stochastic_truth_aux_weight must be >= 0.")
    if cfg.adaln_modulation_init_scale <= 0.0:
        raise ValueError("adaln_modulation_init_scale must be > 0.")
    if cfg.output_head_init_scale <= 0.0:
        raise ValueError("output_head_init_scale must be > 0.")
    if cfg.lr_warmup_train_steps < 0:
        raise ValueError("lr_warmup_train_steps must be >= 0.")
    if cfg.reference_proposals < 1 or cfg.reference_max_proposals < cfg.reference_proposals:
        raise ValueError("reference_max_proposals must be >= reference_proposals >= 1.")
    if cfg.reference_min_ess <= 0.0:
        raise ValueError("reference_min_ess must be > 0.")
...
def _canonicalize_active_sources_before_interpolation_np(
    theta: np.ndarray,
    cfg: BayesTransportConfig = CFG,
) -> np.ndarray:
    """Canonicalize active source rows before synthetic interpolation.

    If both endpoints are sorted by first coordinate, convex interpolation preserves that
    ordering for every tau in [0,1]. This removes arbitrary label-pairing/source-crossing noise.
    """
    theta = np.asarray(theta, dtype=np.float32)
    if not cfg.canonicalize_particle_sources or theta.shape[-2] <= 1:
        return theta
    order = np.argsort(theta[..., 0], axis=-1)
    return np.take_along_axis(theta, order[..., None], axis=-2).astype(np.float32)


def _softmax_weights_np(log_weights: np.ndarray) -> np.ndarray:
    """Stable 1-D softmax used for finite empirical-posterior weights."""
    log_weights = np.asarray(log_weights, dtype=np.float64).reshape(-1)
    shifted = log_weights - np.max(log_weights)
    weights = np.exp(shifted)
    total = weights.sum()
    if not np.isfinite(total) or total <= 0.0:
        return np.full(len(log_weights), 1.0 / len(log_weights), dtype=np.float32)
    return (weights / total).astype(np.float32)


def empirical_posterior_weights_by_prefix_np(
    prior_particles: np.ndarray,
    observations: np.ndarray,
    cfg: BayesTransportConfig = CFG,
) -> np.ndarray:
    """Exact finite-prior posterior weights for every direct observation prefix."""
    prior_particles = np.asarray(prior_particles, dtype=np.float32)
    observations = np.asarray(observations, dtype=np.float32)
    D = prior_particles.shape[-1]
    designs = observations[:, :D]
    readings = observations[:, -1]
    predicted = source_log_mean_np(prior_particles, designs, cfg)
    residual = (readings[None, :] - predicted) / float(cfg.observation_noise_std)
    cumulative_log_likelihood = np.cumsum(-0.5 * residual**2, axis=1)
    prefix_counts = np.arange(
        cfg.min_observations_per_step,
        cfg.max_observations_per_step + 1,
        dtype=np.int32,
    )
    return np.stack(
        [_softmax_weights_np(cumulative_log_likelihood[:, int(o) - 1]) for o in prefix_counts],
        axis=0,
    ).astype(np.float32)
...
def sample_interpolated_training_prior_and_truth_np(
    ...
    *,
    ...
    ``empirical`` draws theta* exactly from the realised empirical prior and is the
    recommended/default mode for the stronger Bayes-operator objective.
    ...
    # Interpolate in the same quotient-space representation later consumed by the model.
    base_particles = _canonicalize_active_sources_before_interpolation_np(base_particles, cfg)
    potential_theta = _canonicalize_active_sources_before_interpolation_np(
        potential_theta[None, ...], cfg
    )[0]
    ...
    if mode == "closed_form":
        ...
        base_truth = _canonicalize_active_sources_before_interpolation_np(
            base_truth[None, ...], cfg
        )[0]
        ...
        # Literal empirical-prior draw: conditional on C, theta* is uniform over the N joint
        # source configurations. This gives an exact finite posterior after observing y.
        particle_index = int(rng.integers(0, n_particles))
        theta_true = prior_particles[particle_index].copy()
...
def simulate_trajectories(
    ...
    *,
    ...
    # Simulation remains host-side.  The stronger training loss evaluates the same known
    # likelihood only to form exact finite-cloud posterior weights on model-generated rollout
    # priors; it is never used to generate those priors.
...
def _simulate_training_rollout_observations_np(
    rng: np.random.Generator,
    theta_true: np.ndarray,
    cfg: BayesTransportConfig = CFG,
) -> np.ndarray:
    """Generate R fresh observation blocks from one fixed theta* for training self-rollouts."""
    theta_true = np.asarray(theta_true, dtype=np.float32)
    D = theta_true.shape[-1]
    R = int(cfg.training_rollout_length)
    designs = rng.uniform(
        cfg.design_low,
        cfg.design_high,
        size=(R, cfg.max_observations_per_step, D),
    ).astype(np.float32)
    mean = source_log_mean_np(theta_true, designs, cfg)
    readings = (mean + cfg.observation_noise_std * rng.normal(size=mean.shape)).astype(np.float32)
    blocks = np.zeros(
        (R, cfg.max_observations_per_step, cfg.max_source_dim + 1), dtype=np.float32
    )
    blocks[..., :D] = designs
    blocks[..., -1] = readings
    return blocks
...
def simulate_iid_joint_samples(
    ...
    *,
    ...
    """Generate fixed iid validation rows from the same stronger joint law as training."""
    ...
    P = cfg.max_observations_per_step - cfg.min_observations_per_step + 1
    R = cfg.training_rollout_length
    theta_true = np.zeros((n_samples, cfg.max_num_sources, cfg.max_source_dim), dtype=np.float32)
    observations = np.zeros(
        (n_samples, cfg.max_observations_per_step, cfg.max_source_dim + 1), dtype=np.float32
    )
    prior_particles = np.zeros(
        (n_samples, cfg.num_particles, cfg.max_num_sources, cfg.max_source_dim), dtype=np.float32
    )
    posterior_weights_by_prefix = np.full(
        (n_samples, P, cfg.num_particles), 1.0 / cfg.num_particles, dtype=np.float32
    )
    rollout_observations = np.zeros(
        (n_samples, R, cfg.max_observations_per_step, cfg.max_source_dim + 1), dtype=np.float32
    )
    ...
    for m in range(n_samples):
        ...
        active_prior, theta_active = sample_interpolated_training_prior_and_truth_np(
            rng, cfg.num_particles, cfg, num_sources=S, source_dim=D
        )
        rollout_blocks = _simulate_training_rollout_observations_np(rng, theta_active, cfg)
        direct_block = rollout_blocks[0]

        theta_true[m, :S, :D] = theta_active
        observations[m] = direct_block
        prior_particles[m] = pad_theta_np(active_prior, cfg)
        rollout_observations[m] = rollout_blocks

        if cfg.training_objective_mode == "empirical_expected_energy":
            posterior_weights_by_prefix[m] = empirical_posterior_weights_by_prefix_np(
                active_prior, direct_block, cfg
            )
    ...
    return {
        ...
        "posterior_weights_by_prefix": posterior_weights_by_prefix,
...
def make_iid_batch_np(
    ...
    *,
    ...
    for optional_name in (
        "posterior_weights_by_prefix",
        "rollout_observations",
    ):
        if optional_name in dataset:
            result[optional_name] = np.asarray(dataset[optional_name][indices], dtype=np.float32)
    return result
...
class ContinuousJointDataset(IterableDataset):
    ...
    metadata are yielded.  The direct row is iid; an additional short stop-gradient rollout is supplied for training exposure.
    """
    ...
    def __iter__(self):
        ...
        while True:
            ...
            rollout_observations = _simulate_training_rollout_observations_np(
                rng, theta_active, self.cfg
            )
            observations = rollout_observations[0]

            if self.cfg.training_objective_mode == "empirical_expected_energy":
                posterior_weights_by_prefix = empirical_posterior_weights_by_prefix_np(
                    active_prior, observations, self.cfg
                )
            else:
                P = self.cfg.max_observations_per_step - self.cfg.min_observations_per_step + 1
                posterior_weights_by_prefix = np.full(
                    (P, self.cfg.num_particles), 1.0 / self.cfg.num_particles, dtype=np.float32
                )
...
class LikelihoodSequenceEmbedder(eqx.Module):
    """Build the observation context consumed by the Posterior Transformer.

    For genuine observation sequences, output token o can depend only on observations 0,...,o
    through the original causal Transformer.

    When Omin=Omax=1 there is no temporal sequence.  The historical `identity` mode therefore
    returns the normalized physical observation token unchanged and owns zero arrays.  The
    stronger `mlp` mode instead learns a small nonlinear context map

        normalized (x,y) -> hidden -> hidden,

    which is important for the nonlinear inverse-distance localisation likelihood while keeping
    exactly the same observation data and downstream conditioning architecture.
    """

    input_projection: eqx.nn.Linear | None
    single_projection_out: eqx.nn.Linear | None
    ...
    single_observation: bool = eqx.field(static=True)
    single_observation_mode: str = eqx.field(static=True)
    ...
    def __init__(
        ...
        *,
        ...
        self.single_observation_mode = str(cfg.single_observation_context_mode)

        if self.single_observation:
            self.attention_heads = 0
            self.blocks = ()
            if self.single_observation_mode == "identity":
                self.hidden_dim = self.input_dim
                self.input_projection = None
                self.single_projection_out = None
                self.final_norm = None
                return

            if self.single_observation_mode != "mlp":
                raise ValueError(
                    f"Unsupported single_observation_context_mode={self.single_observation_mode!r}."
                )
            self.hidden_dim = int(cfg.likelihood_hidden_dim)
            in_key, out_key = jax.random.split(key, 2)
            self.input_projection = eqx.nn.Linear(
                self.input_dim, self.hidden_dim, key=in_key
            )
            self.single_projection_out = eqx.nn.Linear(
                self.hidden_dim, self.hidden_dim, key=out_key
            )
    ...
    def __call__(self, pair_embeddings: Array) -> Array:
        if self.single_observation:
            if pair_embeddings.shape[0] != 1:
                raise ValueError(
                    "Single-observation context encoder expects exactly one observation token."
                )
            if self.single_observation_mode == "identity":
                return pair_embeddings
            if (
                self.input_projection is None
                or self.single_projection_out is None
                or self.final_norm is None
            ):
                raise RuntimeError("Single-observation MLP is missing learned layers.")
            tokens = jax.nn.silu(_linear_tokens(self.input_projection, pair_embeddings))
            tokens = _linear_tokens(self.single_projection_out, tokens)
...
class AdaLNParticleBlock(eqx.Module):
    """Particle self-attention block conditioned through near-zero AdaLN modulation.

    The historical exact-zero gates are replaced by a small configurable initialization so
    observation-dependent gradients reach the conditioning pathway from the first train step.
    ...
    def __init__(
        ...
        *,
        ...
        # Near-zero, NOT exactly zero. Exact double-zero initialization blocked observation
        # gradients at train step 1 and created an identity bootstrap trap.
        modulation = eqx.tree_at(
            lambda layer: layer.weight,
            modulation,
            modulation_init_scale * modulation.weight,
        )
        modulation = eqx.tree_at(
...
def empirical_posterior_weights_compact_jax(
    ...
    """Exact one-step likelihood weights on the CURRENT compact empirical prior cloud.

    This is used only for self-rollout training targets.  It does not construct the prior:
    `compact_particles` is the stop-gradient model-generated cloud that actually enters the
    current Bayes update.
    """
    D = theta_size // num_sources
    padded = jax.vmap(
    ...
    )(compact_particles)  # [N,Smax,Dmax]

    coordinate_mask = (
        jnp.arange(cfg.max_source_dim, dtype=compact_particles.dtype) < D
    ).astype(compact_particles.dtype)
    source_mask = (
        jnp.arange(cfg.max_num_sources, dtype=compact_particles.dtype) < num_sources
    ).astype(compact_particles.dtype)

    designs = observations[:, : cfg.max_source_dim]
    readings = observations[:, -1]
    differences = (
        padded[:, None, :, :] - designs[None, :, None, :]
    )
    dist_sq = jnp.sum(
        differences**2 * coordinate_mask[None, None, None, :], axis=-1
    )
    intensity = cfg.background + jnp.sum(
        cfg.source_strength
        / (cfg.softening + dist_sq)
        * source_mask[None, None, :],
        axis=-1,
    )
    predicted_means = jnp.log(intensity)  # [N,Omax]

    observation_mask = (
        jnp.arange(observations.shape[0]) < observation_count
    ).astype(compact_particles.dtype)
    residual = (readings[None, :] - predicted_means) / cfg.observation_noise_std
    log_weights = -0.5 * jnp.sum(
        residual**2 * observation_mask[None, :], axis=1
    )
...
class ThetaParticleOutputHead(eqx.Module):
    ...
    enforce_uniform_support: bool = eqx.field(static=True)
    support_low: float = eqx.field(static=True)
    support_high: float = eqx.field(static=True)

    def __init__(self, cfg: BayesTransportConfig, *, key: Array):
        self.max_displacement = cfg.max_embedding_displacement
        self.enforce_uniform_support = bool(
            cfg.enforce_uniform_prior_support and cfg.base_prior_distribution == "uniform"
        )
        self.support_low = float(cfg.design_low)
        self.support_high = float(cfg.design_high)
        ...
        output = eqx.tree_at(
            lambda layer: layer.weight,
            output,
            cfg.output_head_init_scale * output.weight,
        )
    ...
    def __call__(
        ...
        updated = current_theta + displacement
        if self.enforce_uniform_support:
            updated = jnp.clip(updated, self.support_low, self.support_high)
        valid = jnp.arange(current_theta.shape[-1]) < theta_size
        return jnp.where(valid[None, :], updated, 0.0)
        ...
        *,
        ...
        self.blocks = tuple(
            AdaLNParticleBlock(
                ...
                cfg.heads,
                cfg.mlp_ratio * cfg.hidden_dim,
                cfg.adaln_modulation_init_scale,
                key=keys[1 + i],
            )
            for i in range(cfg.posterior_depth)
        )
...
#%% 8) End-to-end amortized model with direct training, short self-rollouts, and held-out recurrence
class SequentialBayesModel(eqx.Module):
    """Shared one-step Bayes transport used in three deliberately separated ways.

    `predict_prefixes` keeps the historical direct iid path: every observation prefix starts
    from the same supplied synthetic cloud. `predict_training_rollout` adds short recurrent
    exposure to the model's own posterior-shaped clouds, normally detaching the carry between
    updates so each step receives a local gradient without long-horizon BPTT. `__call__` retains
    the ordinary held-out repeated-Bayes interface used by sequential evaluation.
    """
    ...
    def __init__(self, cfg: BayesTransportConfig, *, key: Array):
        ...
        if self.fixed_shape:
            ...
            # with one observation they enter the configured identity/MLP context encoder;
            # otherwise they enter the causal likelihood Transformer's learned projection.
        ...
        # In single-observation identity mode this equals observation_input_dim; in the stronger
        # single-observation MLP and in sequence mode it equals likelihood_hidden_dim.
    ...
    def _encode_observation_block(
        ...
        # With Omin=Omax=1 this is either the historical identity map or the stronger learned
        # nonlinear context MLP, according to single_observation_context_mode.
    ...
    def predict_prefixes(
        ...
        """Direct training path: one supplied prior -> posterior for EVERY prefix.

        All prefix transports share the exact same supplied input cloud. With Omin=Omax=1,
        the single observation uses the configured identity or nonlinear MLP context encoder;
        otherwise the causal observation Transformer is evaluated once.
    ...
    def predict_training_rollout(
        self,
        prior_particles: Array,
        rollout_observations: Array,
        observation_count: Array,
        num_sources: Array,
        theta_size: Array,
        *,
        stop_gradient_between_steps: bool,
    ) -> tuple[Array, Array, Array]:
        """Short recurrent exposure with optional stop-gradient carry between updates."""
        prior_theta = self._compact_reference_cloud(prior_particles, num_sources, theta_size)
        ...
        def scan_step(current_theta: Array, contexts: Array):
            # current_theta is exactly the empirical prior seen by this local update.
            current_input = current_theta
            ...
            carry = jax.lax.stop_gradient(next_theta) if stop_gradient_between_steps else next_theta
            return carry, (current_input, next_theta)

        _, (input_sequence, output_sequence) = jax.lax.scan(
            scan_step, prior_theta, observation_contexts
        )
        return output_sequence, input_sequence, prior_theta
...
def print_model_parameter_count(model: SequentialBayesModel):
    ...
    if (
        model.single_observation_direct
        and model.likelihood_embedder.single_observation_mode == "identity"
        and likelihood_embedder != 0
    ):
        raise RuntimeError(
            "single_observation_context_mode='identity' must have zero likelihood-context parameters."
        )
    ...
    if model.single_observation_direct:
        print(
            "    single-observation context: "
            f"{model.likelihood_embedder.single_observation_mode}; width={model.observation_context_dim}"
        )
...
def weighted_empirical_energy_score_terms_single(
    particle_theta: Array,
    target_support: Array,
    target_weights: Array,
    theta_size: Array,
) -> tuple[Array, Array, Array]:
    """Exact conditional expected ES against a weighted finite posterior."""
    valid = (jnp.arange(particle_theta.shape[-1]) < theta_size).astype(particle_theta.dtype)
    weights = jnp.asarray(target_weights, dtype=particle_theta.dtype)
    weights = weights / jnp.maximum(jnp.sum(weights), 1e-12)

    cross = particle_theta[:, None, :] - target_support[None, :, :]
    cross_sq = jnp.sum(cross**2 * valid[None, None, :], axis=-1)
    attraction = jnp.mean(
        jnp.sum(jnp.sqrt(cross_sq + 1e-12) * weights[None, :], axis=1)
    )
...
def weighted_target_mean_rmse_single(
    particle_theta: Array,
    target_support: Array,
    target_weights: Array,
    theta_size: Array,
) -> Array:
    """RMSE between learned mean and exact finite-posterior mean."""
    valid = (jnp.arange(particle_theta.shape[-1]) < theta_size).astype(particle_theta.dtype)
    weights = target_weights / jnp.maximum(jnp.sum(target_weights), 1e-12)
    target_mean = jnp.sum(target_support * weights[:, None], axis=0)
    error = (jnp.mean(particle_theta, axis=0) - target_mean) ** 2
    return jnp.sqrt(jnp.sum(error * valid) / jnp.maximum(theta_size, 1))


def _weighted_sequence_metrics(
    predicted_sequence: Array,
    target_support: Array,
    target_weights_sequence: Array,
    theta_size: Array,
) -> tuple[Array, Array, Array, Array]:
    score, attraction, repulsion = jax.vmap(
        lambda particles, weights: weighted_empirical_energy_score_terms_single(
            particles, target_support, weights, theta_size
        )
    )(predicted_sequence, target_weights_sequence)
    target_mean_rmse = jax.vmap(
        lambda particles, weights: weighted_target_mean_rmse_single(
            particles, target_support, weights, theta_size
        )
    )(predicted_sequence, target_weights_sequence)
    return score, attraction, repulsion, target_mean_rmse

def _weighted_local_sequence_metrics(
    predicted_sequence: Array,
    target_support_sequence: Array,
    target_weights_sequence: Array,
    theta_size: Array,
) -> tuple[Array, Array, Array, Array]:
    """Weighted ES when every rollout step has its own current-cloud target support."""
    score, attraction, repulsion = jax.vmap(
        lambda particles, support, weights: weighted_empirical_energy_score_terms_single(
            particles, support, weights, theta_size
        )
    )(predicted_sequence, target_support_sequence, target_weights_sequence)
    target_mean_rmse = jax.vmap(
        lambda particles, support, weights: weighted_target_mean_rmse_single(
            particles, support, weights, theta_size
        )
    )(predicted_sequence, target_support_sequence, target_weights_sequence)
    return score, attraction, repulsion, target_mean_rmse
...
def batch_objective(
    ...
    """Direct conditional-energy target plus posterior-shaped stop-gradient self-rollouts."""
    predicted, _, prior_theta, prefix_counts = jax.vmap(
    ...
    sample_energy, sample_attraction, sample_repulsion, rmse, spread = jax.vmap(_prefix_metrics)(
        predicted, target_theta, batch["theta_size"]
    )

    if cfg.training_objective_mode == "empirical_expected_energy":
        direct_energy, direct_attraction, direct_repulsion, target_mean_rmse = jax.vmap(
            _weighted_sequence_metrics
        )(
            predicted,
            prior_theta,
            batch["posterior_weights_by_prefix"],
            batch["theta_size"],
        )
        direct_teacher_loss = jnp.mean(direct_energy)
        direct_truth_aux_loss = jnp.mean(sample_energy)
        direct_loss = direct_teacher_loss + cfg.stochastic_truth_aux_weight * direct_truth_aux_loss
        energy_for_reporting = direct_energy
        attraction_for_reporting = direct_attraction
        repulsion_for_reporting = direct_repulsion
    else:
        direct_energy = sample_energy
        direct_teacher_loss = jnp.asarray(0.0, dtype=sample_energy.dtype)
        direct_truth_aux_loss = jnp.mean(sample_energy)
        direct_loss = direct_truth_aux_loss
        energy_for_reporting = sample_energy
        attraction_for_reporting = sample_attraction
        repulsion_for_reporting = sample_repulsion
        target_mean_rmse = rmse

    rollout_sequence, rollout_inputs, rollout_prior_theta = jax.vmap(
        lambda prior, obs, sources, size: model.predict_training_rollout(
            prior,
            obs,
            jnp.asarray(cfg.training_rollout_observations_per_step, dtype=jnp.int32),
            sources,
            size,
            stop_gradient_between_steps=cfg.training_rollout_stop_gradient_between_steps,
        )
    )(
        batch["prior_particles"],
        batch["rollout_observations"],
        batch["num_sources"],
        batch["theta_size"],
    )

    sample_rollout_energy, _, _ = jax.vmap(_trajectory_metrics)(
        rollout_sequence, target_theta, batch["theta_size"]
    )

    if cfg.training_objective_mode == "empirical_expected_energy":
        # Targets are teachers, not optimization variables: even if someone disables
        # stop-gradient recurrence, do not differentiate through the empirical prior support
        # or through likelihood-derived target weights.
        rollout_target_support = jax.lax.stop_gradient(rollout_inputs)
        rollout_weights = jax.vmap(
            lambda inputs, blocks, sources, size: jax.vmap(
                lambda current, block: empirical_posterior_weights_compact_jax(
                    current,
                    block,
                    jnp.asarray(cfg.training_rollout_observations_per_step, dtype=jnp.int32),
                    sources,
            ...
            rollout_target_support,
            batch["rollout_observations"],
            batch["num_sources"],
            batch["theta_size"],
        )
        rollout_energy, _, _, rollout_target_mean_rmse = jax.vmap(
            _weighted_local_sequence_metrics
        )(
            rollout_sequence,
            rollout_target_support,
            rollout_weights,
        ...
        rollout_energy = sample_rollout_energy
        rollout_target_mean_rmse = jax.vmap(
            lambda seq, truth, size: jax.vmap(
                lambda particles: posterior_mean_rmse_single(particles, truth, size)
            )(seq)
        )(rollout_sequence, target_theta, batch["theta_size"])

    if cfg.training_rollout_length > 1 and cfg.training_rollout_loss_weight > 0.0:
        if cfg.training_objective_mode == "empirical_expected_energy":
            rollout_teacher_loss = jnp.mean(rollout_energy[:, 1:])
            rollout_truth_aux_loss = jnp.mean(sample_rollout_energy[:, 1:])
            rollout_loss = (
                rollout_teacher_loss
                + cfg.stochastic_truth_aux_weight * rollout_truth_aux_loss
            )
        else:
            rollout_teacher_loss = jnp.asarray(0.0, dtype=direct_loss.dtype)
            rollout_truth_aux_loss = jnp.mean(sample_rollout_energy[:, 1:])
            rollout_loss = rollout_truth_aux_loss
    else:
        rollout_teacher_loss = jnp.asarray(0.0, dtype=direct_loss.dtype)
        rollout_truth_aux_loss = jnp.asarray(0.0, dtype=direct_loss.dtype)
        rollout_loss = jnp.asarray(0.0, dtype=direct_loss.dtype)

    loss = direct_loss + cfg.training_rollout_loss_weight * rollout_loss

    direct_final = predicted[:, -1]
    valid = (
        jnp.arange(direct_final.shape[-1])[None, None, :]
        < batch["theta_size"][:, None, None]
    ).astype(direct_final.dtype)
    particle_displacement = jnp.sqrt(
        jnp.sum((direct_final - prior_theta) ** 2 * valid, axis=-1) + 1e-12
    )
    mean_displacement = jnp.mean(particle_displacement)

    metrics = {
        "loss": loss,
        "direct_loss": direct_loss,
        "direct_teacher_loss": direct_teacher_loss,
        "direct_truth_aux_loss": direct_truth_aux_loss,
        "rollout_loss": rollout_loss,
        "rollout_teacher_loss": rollout_teacher_loss,
        "rollout_truth_aux_loss": rollout_truth_aux_loss,
        "energy_score": jnp.mean(energy_for_reporting),
        "sample_energy_score": jnp.mean(sample_energy),
        "final_energy_score": jnp.mean(energy_for_reporting[:, -1]),
        "posterior_mean_rmse": jnp.mean(rmse),
        "final_mean_rmse": jnp.mean(rmse[:, -1]),
        "target_mean_rmse": jnp.mean(target_mean_rmse),
        "final_target_mean_rmse": jnp.mean(target_mean_rmse[:, -1]),
        "rollout_target_mean_rmse": jnp.mean(rollout_target_mean_rmse),
        "posterior_spread": jnp.mean(spread),
        "final_spread": jnp.mean(spread[:, -1]),
        "attraction": jnp.mean(attraction_for_reporting),
        "repulsion": jnp.mean(repulsion_for_reporting),
        "mean_displacement": mean_displacement,
        "energy_by_o": jnp.mean(energy_for_reporting, axis=0),
        "rmse_by_o": jnp.mean(rmse, axis=0),
        "target_mean_rmse_by_o": jnp.mean(target_mean_rmse, axis=0),
        "spread_by_o": jnp.mean(spread, axis=0),
        "rollout_energy_by_t": jnp.mean(rollout_energy, axis=0),
        "rollout_target_mean_rmse_by_t": jnp.mean(rollout_target_mean_rmse, axis=0),
...
def evaluate_amortized_model(
    ...
    *,
    ...
    scalar_names = [
        "loss", "direct_loss", "direct_teacher_loss", "direct_truth_aux_loss",
        "rollout_loss", "rollout_teacher_loss", "rollout_truth_aux_loss",
        "energy_score", "sample_energy_score", "final_energy_score",
        "posterior_mean_rmse", "final_mean_rmse",
        "target_mean_rmse", "final_target_mean_rmse", "rollout_target_mean_rmse",
        "posterior_spread", "final_spread",
        "attraction", "repulsion", "mean_displacement",
    ]
    by_o_names = [
        "energy_by_o", "rmse_by_o", "target_mean_rmse_by_o", "spread_by_o",
        "rollout_energy_by_t", "rollout_target_mean_rmse_by_t",
    ]
...
def reference_posterior_particles_np(
    ...
    """Adaptive base-prior SNIS reference used only for visual/post-hoc diagnostics."""
    ...
    proposal_chunks: list[np.ndarray] = []
    log_weight_chunks: list[np.ndarray] = []
    total = 0
    ess = 0.0
    while total < cfg.reference_max_proposals:
        chunk_size = min(int(cfg.reference_proposals), int(cfg.reference_max_proposals) - total)
        proposals = sample_base_prior_np(rng, chunk_size, cfg, num_sources=S, source_dim=D)
        predicted_means = source_log_mean_np(proposals, designs, cfg)
        residual = (readings[None, :] - predicted_means) / cfg.observation_noise_std
        log_weights = -0.5 * np.sum(residual**2, axis=1)
        proposal_chunks.append(proposals)
        log_weight_chunks.append(np.asarray(log_weights, dtype=np.float64))
        total += chunk_size

        all_log = np.concatenate(log_weight_chunks, axis=0)
        shifted = all_log - np.max(all_log)
        weights = np.exp(shifted)
        weights /= np.maximum(weights.sum(), 1e-300)
        ess = float(1.0 / np.sum(weights**2))
        if ess >= cfg.reference_min_ess:
            break

    all_proposals = np.concatenate(proposal_chunks, axis=0)
    all_log = np.concatenate(log_weight_chunks, axis=0)
    shifted = all_log - np.max(all_log)
    weights = np.exp(shifted)
    weights /= np.maximum(weights.sum(), 1e-300)
    ess = float(1.0 / np.sum(weights**2))
    indices = rng.choice(len(all_proposals), size=cfg.reference_particles, replace=True, p=weights)
    posterior = all_proposals[indices]
...
def plot_architecture_schematic(
    ...
    ax.text(0.01, 0.96, "TRAINING: exact finite-prior Bayes targets + short stop-gradient self-rollouts",
            fontsize=12, fontweight="bold", va="top")
    ax.text(0.01, 0.43, "HELD-OUT EVALUATION: ordinary repeated Bayes from the base prior",
            fontsize=12, fontweight="bold", va="top")

    draw_box((0.02, 0.65), 0.14, 0.20,
             "theta* from configured synthetic-prior law\nobservations from physical likelihood",
             "iid simulator row")
    fixed_shape = (
        cfg.min_num_sources == cfg.max_num_sources
        and cfg.min_source_dim == cfg.max_source_dim
    ...
    if fixed_shape:
        if single_observation_direct:
            context_label = (
                ...
                f"single-observation context: {cfg.single_observation_context_mode}\n"
                f"1 x (D+1)={cfg.max_source_dim + 1} -> context"
            )
        else:
            context_label = (
                f"parameter-free (x,y) normalization\ncausal Transformer\n"
        ...
        if single_observation_direct:
            context_label = (
                ...
                f"single-observation context: {cfg.single_observation_context_mode}\n"
                f"1 x E={cfg.embedding_dim} -> context"
            )
        else:
    ...
    draw_box(
        (0.02, 0.47), 0.14, 0.14,
        f"z_n, theta_tilde ~ rho_0\n{cfg.interpolation_tau_mode}\n"
        f"C_n=(1-tau_n)z_n+tau_n theta_tilde\ntheta*: {truth_mode_label}",
        "synthetic empirical prior",
    )
    ...
    draw_box((0.71, 0.60), 0.25, 0.23,
             ...
             "exact likelihood-reweighted finite-prior target\n"
             f"+ self-rollout R={cfg.training_rollout_length} (stop-gradient carry)",
             "direct + recurrent training targets")

    arrow((0.16, 0.75), (0.20, 0.75))
    arrow((0.37, 0.75), (0.42, 0.73), "all causal prefix signals")
    arrow((0.16, 0.54), (0.42, 0.65), "same empirical prior for direct prefixes")
    arrow((0.64, 0.71), (0.71, 0.71), r"$T_\phi(C; \mathcal{D}_{1:o})$")
    ...
    fig.suptitle(
        "Empirical-prior energy-score Bayes operator with posterior-shaped self-rollouts",
...
def plot_training_diagnostics(
    ...
    """Training-objective diagnostics plus ordinary sequential-Bayes evaluation behaviour."""
    ...
    def plot_raw_and_smooth(ax, x, values, label):
        values = np.asarray(values, dtype=float)
        line = ax.plot(x, np.maximum(values, 1e-12), linewidth=0.70, alpha=0.25, label=label)[0]
        if len(values) >= 20:
            window = max(5, len(values) // 100)
            smooth = np.convolve(values, np.ones(window) / window, mode="valid")
            ax.plot(
                x[window - 1:], np.maximum(smooth, 1e-12), linewidth=2.4, alpha=0.98,
                color=line.get_color(), label=f"{label} moving average ({window})"
            )
        return line

    plot_raw_and_smooth(axes[0, 0], steps, history["step_loss"], "total loss")
    axes[0, 0].set_title("Training objective", loc="left", fontweight="bold")
    axes[0, 0].set_yscale("log")

    plot_raw_and_smooth(axes[0, 1], steps, history["step_direct_loss"], "direct prior→posterior")
    if np.any(np.asarray(history["step_rollout_loss"], dtype=float) > 0):
        plot_raw_and_smooth(axes[0, 1], steps, history["step_rollout_loss"], "self-rollout steps 2:R")
    axes[0, 1].set_title("Where the training signal comes from", loc="left", fontweight="bold")
    axes[0, 1].set_yscale("log")

    plot_raw_and_smooth(
        axes[0, 2], steps, history["step_target_mean_rmse"],
        r"RMSE to exact empirical posterior mean"
    )
    plot_raw_and_smooth(
        axes[0, 2], steps, history["step_mean_rmse"], r"RMSE to sampled $\theta^\star$"
    )
    axes[0, 2].set_title("Posterior-mean diagnostics", loc="left", fontweight="bold")
    axes[0, 2].set_yscale("log")

    grad_line = axes[0, 3].plot(
        steps, history["step_grad_norm"], linewidth=0.70, alpha=0.30, label="gradient norm"
    )[0]
    if len(steps) >= 20:
        window = max(5, len(steps) // 100)
        smooth = np.convolve(np.asarray(history["step_grad_norm"], dtype=float),
                             np.ones(window) / window, mode="valid")
        axes[0, 3].plot(
            steps[window - 1:], smooth, linewidth=2.3, alpha=0.98,
            color=grad_line.get_color(), label=f"gradient norm moving average ({window})"
        )
    move_axis = axes[0, 3].twinx()
    move_axis.plot(
        steps, history["step_mean_displacement"], linewidth=1.0, alpha=0.55,
        linestyle="--", label=r"mean $\|\Delta\theta\|$"
    )
    move_axis.set_ylabel(r"mean $\|\Delta\theta\|$")
    axes[0, 3].set_title("Optimization + update magnitude", loc="left", fontweight="bold")

    formatter = FuncFormatter(_format_train_step_k)
    for ax in axes[0, :]:
    ...
    axes[1, 0].plot(
        epochs, np.maximum(history["epoch_train_loss"], 1e-12), marker="o", markersize=2.8,
        linewidth=1.0, label="train total loss"
    )
    axes[1, 0].plot(
        epochs, np.maximum(history["epoch_val_loss"], 1e-12), marker="o", markersize=2.8,
        linewidth=1.0, label="iid validation total loss"
    )
    axes[1, 0].axvline(best_epoch, linestyle="--", linewidth=1.0, label=f"best epoch {best_epoch}")
    lr_axis = axes[1, 0].twinx()
    lr_axis.plot(
        epochs, np.asarray(history["epoch_learning_rate"], dtype=float), linestyle=":",
        linewidth=1.4, alpha=0.70
    )
    lr_axis.set_yscale("log")
    lr_axis.set_ylabel("learning rate")
    axes[1, 0].set_title("Model selection", loc="left", fontweight="bold")
    axes[1, 0].set_yscale("log")
    axes[1, 0].legend(fontsize=7.2)

    target_rmse_by_o = np.asarray(history["epoch_val_target_mean_rmse_by_o"], dtype=float)
    selected_epochs = np.unique(
        np.clip(np.rint(np.linspace(0, len(target_rmse_by_o) - 1, 5)).astype(int),
                0, len(target_rmse_by_o) - 1)
    )
    ...
    for epoch_index in selected_epochs:
        axes[1, 1].plot(
            observation_axis, np.maximum(target_rmse_by_o[epoch_index], 1e-12),
            marker="o", markersize=3, label=f"epoch {epoch_index + 1}"
        )
    axes[1, 1].set_title("Direct update: exact empirical posterior mean", loc="left", fontweight="bold")
    axes[1, 1].set_yscale("log")
    axes[1, 1].legend(fontsize=7.2)

    train_rollout = np.asarray(history["epoch_train_rollout_target_mean_rmse_by_t"], dtype=float)
    rollout_axis = np.arange(1, train_rollout.shape[1] + 1)
    for epoch_index in selected_epochs:
        axes[1, 2].plot(
            rollout_axis, np.maximum(train_rollout[epoch_index], 1e-12),
            label=f"epoch {epoch_index + 1}"
        )
    axes[1, 2].set_title("Training self-rollout: exact empirical target", loc="left", fontweight="bold")
    axes[1, 2].set_yscale("log")
    axes[1, 2].legend(fontsize=7.2)

    sequential_rmse = np.asarray(history["epoch_val_rmse_by_t"], dtype=float)
    t_axis = np.arange(1, sequential_rmse.shape[1] + 1)
    for epoch_index in selected_epochs:
        axes[1, 3].plot(
            t_axis, np.maximum(sequential_rmse[epoch_index], 1e-12),
            label=f"epoch {epoch_index + 1}"
        )
    axes[1, 3].set_title("Held-out repeated Bayes: RMSE to fixed truth", loc="left", fontweight="bold")
    axes[1, 3].set_yscale("log")
    axes[1, 3].legend(fontsize=7.2)

    for col, ax in enumerate(axes[1, :]):
        ax.set_xlabel("epoch" if col == 0 else ("observations in direct prefix" if col == 1 else "sequential update step"))
        ax.grid(alpha=0.18)

    fig.suptitle(
        f"Energy-score Bayes-operator training + posterior-shaped self-rollouts ({cfg.posterior_conditioning})",
        fontsize=14, fontweight="bold"
    )
    if destination is not None:
...
def train_model(
    ...
    """Train the one-step Bayes operator with direct targets and short stop-gradient rollouts.

    In the recommended empirical mode the finite input cloud is the literal prior, so
    likelihood reweighting supplies the exact conditional expected energy-score target for
    every direct prefix; self-rollout steps compute the exact one-step target on their own
    stop-gradient current empirical cloud.

    The rollout recurrence is used for training exposure, but by default its carry is detached
    between steps. There is therefore no long-horizon BPTT: each step learns the same local
    update on the posterior-shaped cloud it actually receives.

    A separate ordinary base-prior sequential dataset remains held out as the main
    repeated-Bayes generalisation test.
    """
    ...
    history: dict[str, list] = {
        "step_loss": [],
        "step_direct_loss": [],
        "step_direct_teacher_loss": [],
        "step_direct_truth_aux_loss": [],
        "step_rollout_loss": [],
        "step_rollout_teacher_loss": [],
        "step_rollout_truth_aux_loss": [],
        "step_energy_score": [],
        "step_sample_energy_score": [],
        "step_final_energy_score": [],
        "step_mean_rmse": [],
        "step_target_mean_rmse": [],
        "step_mean_displacement": [],
        "step_attraction": [],
        "step_repulsion": [],
        "step_grad_norm": [],
        "epoch_train_loss": [],
        "epoch_learning_rate": [],
        "epoch_val_loss": [],
        "epoch_val_direct_loss": [],
        "epoch_val_direct_teacher_loss": [],
        "epoch_val_direct_truth_aux_loss": [],
        "epoch_val_rollout_loss": [],
        "epoch_val_rollout_teacher_loss": [],
        "epoch_val_rollout_truth_aux_loss": [],
        "epoch_val_energy_score": [],
        "epoch_val_sample_energy_score": [],
        "epoch_val_final_energy_score": [],
        "epoch_val_mean_rmse": [],
        "epoch_val_target_mean_rmse": [],
        "epoch_val_mean_displacement": [],
        "epoch_val_attraction": [],
        "epoch_val_repulsion": [],
        "epoch_val_energy_by_o": [],
        "epoch_val_rmse_by_o": [],
        "epoch_val_target_mean_rmse_by_o": [],
        "epoch_val_spread_by_o": [],
        "epoch_train_rollout_energy_by_t": [],
        "epoch_train_rollout_target_mean_rmse_by_t": [],
    ...
    print(
        f"[amortized] initial loss={initial_iid['loss']:.6f} | "
        f"direct={initial_iid['direct_loss']:.6f} | rollout={initial_iid['rollout_loss']:.6f} | "
        f"target-mean RMSE={initial_iid['target_mean_rmse']:.5f} || sequential final ES="
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for _ in progress:
            ...
            warmup_scale = (
                1.0
                if cfg.lr_warmup_train_steps == 0
                else min(1.0, float(global_step + 1) / float(cfg.lr_warmup_train_steps))
            )
            effective_update_scale = epoch_lr_scale * jnp.asarray(warmup_scale, dtype=jnp.float32)
            model, opt_state, loss, metrics, grad_norm = train_step(
                model, opt_state, effective_update_scale, batch
            )
            ...
            history["step_direct_loss"].append(float(host["direct_loss"]))
            history["step_direct_teacher_loss"].append(float(host["direct_teacher_loss"]))
            history["step_direct_truth_aux_loss"].append(float(host["direct_truth_aux_loss"]))
            history["step_rollout_loss"].append(float(host["rollout_loss"]))
            history["step_rollout_teacher_loss"].append(float(host["rollout_teacher_loss"]))
            history["step_rollout_truth_aux_loss"].append(float(host["rollout_truth_aux_loss"]))
            history["step_energy_score"].append(float(host["energy_score"]))
            history["step_sample_energy_score"].append(float(host["sample_energy_score"]))
            ...
            history["step_target_mean_rmse"].append(float(host["target_mean_rmse"]))
            history["step_mean_displacement"].append(float(host["mean_displacement"]))
            ...
            progress.set_postfix(
                loss=f"{host_loss:.4f}",
                direct=f"{float(host['direct_loss']):.4f}",
                rollout=f"{float(host['rollout_loss']):.4f}",
                refresh=False,
            )
        ...
        history["epoch_val_direct_loss"].append(float(val_metrics["direct_loss"]))
        history["epoch_val_direct_teacher_loss"].append(float(val_metrics["direct_teacher_loss"]))
        history["epoch_val_direct_truth_aux_loss"].append(float(val_metrics["direct_truth_aux_loss"]))
        history["epoch_val_rollout_loss"].append(float(val_metrics["rollout_loss"]))
        history["epoch_val_rollout_teacher_loss"].append(float(val_metrics["rollout_teacher_loss"]))
        history["epoch_val_rollout_truth_aux_loss"].append(float(val_metrics["rollout_truth_aux_loss"]))
        history["epoch_val_energy_score"].append(float(val_metrics["energy_score"]))
        history["epoch_val_sample_energy_score"].append(float(val_metrics["sample_energy_score"]))
        ...
        history["epoch_val_target_mean_rmse"].append(float(val_metrics["target_mean_rmse"]))
        history["epoch_val_mean_displacement"].append(float(val_metrics["mean_displacement"]))
        ...
        history["epoch_val_target_mean_rmse_by_o"].append(
            np.asarray(val_metrics["target_mean_rmse_by_o"], dtype=np.float64)
        )
        history["epoch_val_spread_by_o"].append(np.asarray(val_metrics["spread_by_o"], dtype=np.float64))
        history["epoch_train_rollout_energy_by_t"].append(
            np.asarray(val_metrics["rollout_energy_by_t"], dtype=np.float64)
        )
        history["epoch_train_rollout_target_mean_rmse_by_t"].append(
            np.asarray(val_metrics["rollout_target_mean_rmse_by_t"], dtype=np.float64)
        )
        ...
        save_json(
            ...
            {
                "training": (
                    "iid finite-prior rows with direct exact conditional-energy targets plus "
                    "short stop-gradient self-rollout exposure"
                ),
                "data_assumption": (
                ...
                "single_observation_context_mode": cfg.single_observation_context_mode,
                "posterior_recurrence_in_training": (
                    "short self-rollout exposure; stop-gradient carry between steps"
                    if cfg.training_rollout_length > 1 else False
                ),
                "training_rollout_length": cfg.training_rollout_length,
                "training_rollout_loss_weight": cfg.training_rollout_loss_weight,
                "training_rollout_stop_gradient_between_steps": cfg.training_rollout_stop_gradient_between_steps,
                "sequential_recurrence": "held-out evaluation repeatedly applies the same learned transport",
                "epoch": epoch,
                ...
                "objective": (
                    "exact conditional expected empirical energy score under likelihood-reweighted finite-prior targets"
                    if cfg.training_objective_mode == "empirical_expected_energy"
                    else "historical stochastic single-theta empirical energy score"
                ),
                "training_objective_mode": cfg.training_objective_mode,
                "stochastic_truth_aux_weight": cfg.stochastic_truth_aux_weight,
        ...
        print(
            ...
            f"(direct={float(val_metrics['direct_loss']):.6f}, rollout={float(val_metrics['rollout_loss']):.6f}) | "
            f"target-mean RMSE={float(val_metrics['target_mean_rmse']):.5f} | "
    ...
    print(
        ...
        f"best epoch={best_epoch}, iid val loss={best_val_loss:.6f}"
    )
    return {
        "model": best_model,
        "history": history,
...
def _recover_epoch_history_from_nohup(path: Path | None) -> dict[str, np.ndarray] | None:
    """Recover reliable epoch-level quantities from either stronger or legacy training logs."""
    ...
    stronger = re.compile(
        r"^\[amortized\] epoch\s+(\d+): train loss=([^|\s]+) \| "
        r"val loss=([^\s]+) \(direct=([^,\s]+), rollout=([^\)\s]+)\) \| "
        r"target-mean RMSE=([^|\s]+) \| lr=([^|\s]+) -> ([^|\s]+) \|\| "
        r"seq final ES=([^|\s]+) \| seq final RMSE=([^|\s]+) \| ([^|\s]+)s$",
        flags=re.MULTILINE,
    )
    rows = stronger.findall(log_text)
    if rows:
        v = np.asarray(rows, dtype=object)
        return {
            "epoch": v[:, 0].astype(np.int32),
            "epoch_train_loss": v[:, 1].astype(np.float64),
            "epoch_val_loss": v[:, 2].astype(np.float64),
            "epoch_val_direct_loss": v[:, 3].astype(np.float64),
            "epoch_val_rollout_loss": v[:, 4].astype(np.float64),
            # Keep the historical key name used by the compact recovery plot.
            "epoch_val_mean_rmse": v[:, 5].astype(np.float64),
            "epoch_learning_rate": v[:, 6].astype(np.float64),
            "epoch_next_learning_rate": v[:, 7].astype(np.float64),
            "epoch_seq_final_energy_score": v[:, 8].astype(np.float64),
            "epoch_seq_final_rmse": v[:, 9].astype(np.float64),
            "epoch_seconds": v[:, 10].astype(np.float64),
        }
    ...
    return {
        ...
        "epoch_train_loss": v[:, 1].astype(np.float64),
        "epoch_val_loss": v[:, 2].astype(np.float64),
        "epoch_val_mean_rmse": v[:, 3].astype(np.float64),
        "epoch_learning_rate": v[:, 4].astype(np.float64),
        "epoch_next_learning_rate": v[:, 5].astype(np.float64),
        "epoch_seq_final_energy_score": v[:, 6].astype(np.float64),
        "epoch_seq_final_rmse": v[:, 7].astype(np.float64),
        "epoch_seconds": v[:, 8].astype(np.float64),
    }


def _recover_initial_metrics_from_nohup(path: Path | None) -> dict[str, float] | None:
    """Recover the pre-training evaluation line from stronger or legacy nohup output."""
    if path is None or not path.is_file():
        return None
    try:
        log_text = path.read_text(encoding="utf-8", errors="replace")
    ...
    stronger = re.search(
        r"^\[amortized\] initial loss=([^|\s]+) \| direct=([^|\s]+) \| "
        r"rollout=([^|\s]+) \| target-mean RMSE=([^|\s]+) "
    ...
    if stronger is not None:
        return {
            "initial_iid_energy_score": float(stronger.group(1)),
            "initial_iid_rmse": float(stronger.group(4)),
            "initial_seq_final_energy_score": float(stronger.group(5)),
        }
    ...
    if legacy is None:
        return None
    return {
        "initial_iid_energy_score": float(legacy.group(1)),
        "initial_iid_rmse": float(legacy.group(2)),
        "initial_seq_final_energy_score": float(legacy.group(3)),
    }
...
def _full_training_history_available(history: dict[str, np.ndarray] | None) -> bool:
    """Whether the stronger exact training-diagnostics figure can be reproduced."""
    required = {
        "step_loss",
        "step_direct_loss",
        "step_rollout_loss",
...
if train_wm:
    ...
    print(f"  training objective: {CFG.training_objective_mode}; self-rollout length={CFG.training_rollout_length}")
...
#%% 19) Train the Bayes operator, or reload the best available local checkpoint
# Observation embedder + theta embedder + Posterior Transformer are optimized jointly from
# the same physical-theta energy-score family.  The stronger default uses exact conditional
# empirical-posterior targets and a short stop-gradient recurrent exposure path; the ordinary
# long sequential trajectory remains held out for evaluation.
if train_wm:
    result = train_model(
        train_loader,
        amortized_eval_data,
...
summary = {
    ...
    "objective": CFG.training_objective_mode,
    "training_mode": "direct finite-prior Bayes targets + short stop-gradient self-rollouts",
    "data_assumption": (
        "the synthetic prior cloud is generated without the likelihood; by default theta* is an "
        "independent draw from the same interpolated population law and observations are simulated from theta*"
    ),
    ...
    "posterior_recurrence_in_training": CFG.training_rollout_length > 1,
    "training_rollout_length": CFG.training_rollout_length,
    "training_rollout_loss_weight": CFG.training_rollout_loss_weight,
    "training_rollout_stop_gradient_between_steps": CFG.training_rollout_stop_gradient_between_steps,
    ...
    "learning_rate_schedule": {
        "type": "linear train-step warmup + validation reduce_on_plateau",
        "base_learning_rate": CFG.learning_rate,
        "warmup_train_steps": CFG.lr_warmup_train_steps,
        "factor": 0.5,
    ...
    "training_objective_mode": CFG.training_objective_mode,
    "stochastic_truth_aux_weight": CFG.stochastic_truth_aux_weight,
    "adaln_modulation_init_scale": CFG.adaln_modulation_init_scale,
    "output_head_init_scale": CFG.output_head_init_scale,
    "enforce_uniform_prior_support": CFG.enforce_uniform_prior_support,
    "synthetic_prior_match_probability_legacy_only": CFG.synthetic_prior_match_probability,
    ...
    "single_observation_context_mode": CFG.single_observation_context_mode,
    "single_observation_direct": model.single_observation_direct,
    "single_observation_context_encoder_active": (
        (not model.single_observation_direct)
        or CFG.single_observation_context_mode != "identity"
    ),
    "causal_likelihood_transformer_active": not model.single_observation_direct,
