...
For every simulated trajectory m we draw a NEW problem shape, a NEW random finite
Gaussian-mixture prior P_m, and then a generating parameter from that prior

    S_m ~ p(S),    D_m ~ p(D),    P_m ~ p(P | S_m,D_m),    theta_m^* ~ P_m,
...
A configurable SIGReg term acts on theta embeddings drawn across independent random
meta-prior tasks to discourage the shared theta embedder from collapsing to a constant
representation.  It deliberately does NOT Gaussianize each individual input prior,
because those non-Gaussian shapes are now part of the conditioning signal.  Setting
sigreg_weight=0.0 disables it.  No stop-gradient is used anywhere in the end-to-end
inference model.
...
prior_particles      [B, N, Smax, Dmax]   iid from each trajectory's random P_m, padded
...
# Execution switches.
# - train_wm=True: create a new run and train the main model exactly as before.
# - train_wm=False: run this notebook from an existing architecture folder, e.g.
#   runs/<run_name>/cross_attention/.  The saved config is read from ../config.yaml.
...
class BayesTransportConfig:
    ...
    num_sources: int = 2
    source_dim: int = 2
    prior_std: float = 1.0  # fixed theta-value scale used by the embedder
    # Meta-distribution over input priors.  Each trajectory gets one finite Gaussian
    # mixture over the ACTIVE flattened theta in R^(S*D):
    #   K ~ Uniform{prior_min_components,...,prior_max_components}
    #   log(alpha) ~ Normal(prior_log_alpha_mean, prior_log_alpha_std^2)
    #   w ~ Dirichlet(alpha/K,...,alpha/K)
    #   mu_k ~ Normal(0, prior_component_mean_std^2 I)
    #   Sigma_k = A_k A_k^T with a random orientation and random rank.  A rank of
    #   zero is allowed, so components may be exact point masses; intermediate ranks
    #   give exactly singular Gaussians and full rank remains explicitly represented.
    prior_min_components: int = 1
    prior_max_components: int = 8
    prior_log_alpha_mean: float = 0.0
    prior_log_alpha_std: float = 1.0
    prior_component_mean_std: float = 2.0
    prior_log_eigen_std_mean: float = -0.5
    prior_log_eigen_std_std: float = 1.0
    prior_degenerate_covariance_probability: float = 0.01
    ...
    # Optional ablation: when True, each likelihood summary attends only to the
    # current (design, outcome) pair rather than the full prefix history.
    condition_on_current_observation_only: bool = False
...
def validate_config(cfg: BayesTransportConfig):
    ...
    if cfg.prior_min_components < 1:
        raise ValueError("prior_min_components must be >= 1.")
    if cfg.prior_max_components < cfg.prior_min_components:
        raise ValueError("prior_max_components must be >= prior_min_components.")
    if cfg.prior_log_alpha_std < 0.0 or cfg.prior_log_eigen_std_std < 0.0:
        raise ValueError("log-scale standard deviations must be non-negative.")
    if cfg.prior_component_mean_std <= 0.0:
        raise ValueError("prior_component_mean_std must be positive.")
    if not (0.0 <= cfg.prior_degenerate_covariance_probability <= 1.0):
        raise ValueError("prior_degenerate_covariance_probability must lie in [0,1].")
...
# from the saved run config; only decoder_epochs is intentionally taken from this script so
# a decoder can be retrained for a different number of epochs without changing the WM.
_script_cfg = BayesTransportConfig()
if train_wm:
    CFG = _script_cfg
else:
    _variant_dir = Path.cwd().expanduser().resolve()
    _conditioning_name = _variant_dir.name
    if _conditioning_name not in {"adaln", "cross_attention"}:
        raise RuntimeError(
            "With train_wm=False, run the notebook from the existing architecture folder "
            "itself (for example .../cross_attention/)."
        )
    _config_path = _variant_dir.parent / "config.yaml"
    if not _config_path.is_file():
        raise FileNotFoundError(
            f"Could not find the saved run config one level above the current folder: {_config_path}"
        )
    with _config_path.open("r", encoding="utf-8") as handle:
        _saved_cfg_dict = yaml.safe_load(handle)
    for _tuple_field in (
        "architectures_to_train", "particle_limit_values", "trajectory_mc_values"
    ):
    ...
    CFG = replace(_saved_cfg, decoder_epochs=_script_cfg.decoder_epochs)
...
) -> dict[str, np.ndarray | np.int32 | np.float32]:
    """Draw one random finite-Gaussian-mixture prior on flattened active theta.

    The covariance is represented by a factor A with Sigma=A A^T instead of calling a
    multivariate-normal routine.  This makes exact singularity completely explicit:
    with configured probability we sample rank r in {0,...,d-1}; r=0 is a point mass.
    Otherwise r=d.  Non-zero eigen-standard-deviations are log-normal, so arbitrarily
    small and arbitrarily large positive scales both have support.
    """
    d = int(theta_size)
    if d < 1 or d > cfg.embedding_dim:
        raise ValueError(f"theta_size={d} is incompatible with embedding_dim={cfg.embedding_dim}.")

    K = int(rng.integers(cfg.prior_min_components, cfg.prior_max_components + 1))
    alpha = float(np.exp(rng.normal(cfg.prior_log_alpha_mean, cfg.prior_log_alpha_std)))
    active_weights = rng.dirichlet(np.full(K, alpha / K, dtype=np.float64))

    Kmax = cfg.prior_max_components
    dmax = cfg.max_num_sources * cfg.max_source_dim
    weights = np.zeros((Kmax,), dtype=np.float32)
    means = np.zeros((Kmax, dmax), dtype=np.float32)
    factors = np.zeros((Kmax, dmax, dmax), dtype=np.float32)
    weights[:K] = active_weights.astype(np.float32)
    means[:K, :d] = rng.normal(
        0.0, cfg.prior_component_mean_std, size=(K, d)
    ).astype(np.float32)

    for k in range(K):
        if rng.random() < cfg.prior_degenerate_covariance_probability:
            rank = int(rng.integers(0, d))  # exactly 0,...,d-1; rank 0 is a point mass
        else:
            rank = d
        ...
        # QR of an iid Gaussian matrix gives a random orthogonal orientation.  Only
        # the first `rank` columns are active; the remaining factor columns stay zero.
        Q, _ = np.linalg.qr(rng.normal(size=(d, d)))
        eigen_stds = np.exp(
            rng.normal(
                cfg.prior_log_eigen_std_mean,
                cfg.prior_log_eigen_std_std,
                size=rank,
            )
        )
        factors[k, :d, :rank] = (
            Q[:, :rank] * eigen_stds[None, :]
        ).astype(np.float32)

    return {
        "prior_num_components": np.int32(K),
        "prior_alpha": np.float32(alpha),
        "prior_weights": weights,
        "prior_means": means,
        "prior_factors": factors,
    }
...
def sample_prior_np(
    ...
    *,
    ...
    prior_num_components: int | None = None,
    prior_weights: np.ndarray | None = None,
    prior_means: np.ndarray | None = None,
    prior_factors: np.ndarray | None = None,
) -> np.ndarray:
    """Draw n theta samples from one finite-Gaussian-mixture prior.

    If no prior specification is supplied, a fresh random mixture is first sampled from
    the configured meta-distribution.  Training trajectories *do* store their mixture
    specification so theta_true and every later fresh prior cloud come from the same P.
    """
    S = cfg.num_sources if num_sources is None else int(num_sources)
    D = cfg.source_dim if source_dim is None else int(source_dim)
    d = S * D
    if d > cfg.embedding_dim:
        raise ValueError(f"theta size {d} exceeds embedding_dim={cfg.embedding_dim}.")

    supplied = (
        prior_num_components is not None,
        prior_weights is not None,
        prior_means is not None,
        prior_factors is not None,
    )
    if any(supplied) and not all(supplied):
        raise ValueError("Supply either all mixture-prior parameters or none of them.")
    if not all(supplied):
        spec = sample_prior_spec_np(rng, d, cfg)
        prior_num_components = int(spec["prior_num_components"])
        prior_weights = np.asarray(spec["prior_weights"])
        prior_means = np.asarray(spec["prior_means"])
        prior_factors = np.asarray(spec["prior_factors"])

    K = int(prior_num_components)
    weights = np.asarray(prior_weights[:K], dtype=np.float64)
    weights = weights / np.maximum(weights.sum(), 1e-300)
    component = rng.choice(K, size=int(n), replace=True, p=weights)

    active_means = np.asarray(prior_means, dtype=np.float64)[component, :d]
    active_factors = np.asarray(prior_factors, dtype=np.float64)[component, :d, :d]
    eps = rng.normal(size=(int(n), d))
    flat = active_means + np.einsum("nij,nj->ni", active_factors, eps)
    return flat.reshape(int(n), S, D).astype(np.float32)
...
def simulate_mode_a_trajectories(
    ...
    *,
    ...
    ----------------------------
    1. Each row m receives its own S_m and D_m and its own random finite Gaussian
       mixture P_m, then theta_true[m] is drawn once from P_m(theta | S_m,D_m).
    ...
    4. Prior particles are deliberately NOT stored here.  Only P_m's mixture
       parameters are stored; fresh particles are re-drawn from that SAME P_m whenever
       a minibatch is formed.
    ...
    prior_alpha = np.zeros((n_trajectories,), dtype=np.float32)
    prior_weights = np.zeros(
        (n_trajectories, cfg.prior_max_components), dtype=np.float32
    )
    prior_means = np.zeros(
        (n_trajectories, cfg.prior_max_components, max_theta_size), dtype=np.float32
    )
    prior_factors = np.zeros(
        (
            n_trajectories,
            cfg.prior_max_components,
            max_theta_size,
            max_theta_size,
        ),
        dtype=np.float32,
    ...
    for m in range(n_trajectories):
        ...
        spec = sample_prior_spec_np(rng, int(theta_size[m]), cfg)
        prior_num_components[m] = spec["prior_num_components"]
        prior_alpha[m] = spec["prior_alpha"]
        prior_weights[m] = spec["prior_weights"]
        prior_means[m] = spec["prior_means"]
        prior_factors[m] = spec["prior_factors"]
        ...
        theta_active = sample_prior_np(
            ...
            num_sources=S,
            source_dim=D,
            prior_num_components=int(prior_num_components[m]),
            prior_weights=prior_weights[m],
            prior_means=prior_means[m],
            prior_factors=prior_factors[m],
        )[0]
        designs = rng.uniform(
            cfg.design_low,
            cfg.design_high,
    ...
    return {
        ...
        # Store the random prior itself so every fresh numerical prior cloud seen in
        # later epochs is re-sampled from the SAME trajectory-specific distribution.
...
def make_batch_np(
    ...
    *,
    ...
    The prior cloud and theta_true are both sampled independently from the SAME stored
    trajectory-specific random mixture P_m.  The prior cloud is an INPUT numerical
    measure; theta_true is the
    ...
    for b, (S_value, theta_size_value) in enumerate(
        ...
        active = sample_prior_np(
            ...
            num_sources=S,
            source_dim=D,
            prior_num_components=int(dataset["prior_num_components"][row]),
            prior_weights=dataset["prior_weights"][row],
            prior_means=dataset["prior_means"][row],
            prior_factors=dataset["prior_factors"][row],
        )
...
class CausalPrefixLikelihoodTransformer(eqx.Module):
    ...
    condition_on_current_observation_only: bool = eqx.field(static=True)

    def __init__(self, cfg: BayesTransportConfig, *, key: Array):
        keys = jax.random.split(key, cfg.likelihood_depth + 5)
        self.count_scale = float(max(cfg.trajectory_length, 1))
        self.condition_on_current_observation_only = cfg.condition_on_current_observation_only
    ...
    def __call__(self, observations: Array, num_sources: Array, theta_size: Array) -> Array:
        ...
        # Row t is the membership mask of observations 1,...,t+1.  In the optional
        # current-observation-only ablation, replace the causal triangle by the identity
        # so summary t can attend only to observation t.
        prefix_masks = jnp.tril(
            jnp.ones((trajectory_length, trajectory_length), dtype=bool)
        )
        if self.condition_on_current_observation_only:
            prefix_masks = jnp.eye(trajectory_length, dtype=bool)
        normalized_counts = (
            jnp.arange(1, trajectory_length + 1, dtype=observations.dtype)
            / self.count_scale
        )[:, None]
        # If conditioning literally only on the current pair, do not leak prefix length
        # through the count token either.
        if self.condition_on_current_observation_only:
            normalized_counts = jnp.full_like(normalized_counts, 1.0 / self.count_scale)
...
def print_model_parameter_count(model: ModeAParallelBayesModel):
    likelihood = count_parameters(model.likelihood_transformer)
    ...
    total = likelihood + theta_embedder + posterior + sigreg

    print(f"Total WM parameters: {total / 1e6:.3f} M")
    print(f"  Likelihood Transformer : {likelihood:,}")
...
class SIGReg(eqx.Module):
    ...
    Input z has shape (T,B,D).  With the meta-distribution over priors we feed one
    theta_true embedding from each independent trajectory as z=[1, minibatch, E].  This
    regularizes the META-prior marginal rather than forcing every individual random input
    distribution to become N(0,I), which would erase the very prior-shape information the
    sequential updater is meant to use.  Posterior clouds are NOT constrained to Gaussian.
...
def batch_objective(
    ...
    if cfg.sigreg_weight > 0.0:
        # With random input priors, SIGReg must NOT force every individual prior cloud
        # to look N(0,I), because that would erase precisely the distributional
        # variation the posterior updater needs to read.  theta_true provides one
        # independent draw from each trajectory-specific random prior, so across the
        # minibatch it samples the META-prior marginal while preserving per-prior shape.
        sigreg_loss = model.sigreg(target_embeddings[None, :, :], sigreg_key)
    else:
...
def reference_posterior_particles_np(
    ...
    cfg: BayesTransportConfig = CFG,
    *,
    prior_num_components: int,
    prior_weights: np.ndarray,
    prior_means: np.ndarray,
    prior_factors: np.ndarray,
...
def plot_reference_comparison(
    ...
    reference, ess = reference_posterior_particles_np(
        ...
        theta_size,
        cfg,
        prior_num_components=int(trajectory["prior_num_components"]),
        prior_weights=np.asarray(trajectory["prior_weights"]),
        prior_means=np.asarray(trajectory["prior_means"]),
        prior_factors=np.asarray(trajectory["prior_factors"]),
    )
    learned = {}
...
def plot_training_diagnostics(
    ...
    if len(values) >= 20:
        ...
        ## Make the color same as the total loss line, but denser and less transparent. Denser in color and less transparent.
        axes[0, 0].plot(steps[window - 1:], smoothed, linewidth=1.8, color="C0", alpha=0.95, label=f"total moving average ({window})")
...
#%% 20) Create a new run OR reload one existing architecture folder
np.random.seed(CFG.seed)
print("JAX devices:", jax.devices())
print("Configuration:\n", yaml.safe_dump(asdict(CFG), sort_keys=False))

if train_wm:
    # Original full-training path.
    ...
    fixed_trajectory = {
        ...
        "prior_num_components": fixed_data["prior_num_components"][0],
        "prior_alpha": fixed_data["prior_alpha"][0],
        "prior_weights": fixed_data["prior_weights"][0],
        "prior_means": fixed_data["prior_means"][0],
        "prior_factors": fixed_data["prior_factors"][0],
    }
    fixed_prior_active = sample_prior_np(
        ...
        prior_num_components=int(fixed_trajectory["prior_num_components"]),
        prior_weights=fixed_trajectory["prior_weights"],
        prior_means=fixed_trajectory["prior_means"],
        prior_factors=fixed_trajectory["prior_factors"],
    )
    ...
    np.savez_compressed(
        ...
        prior_num_components=fixed_trajectory["prior_num_components"],
        prior_alpha=fixed_trajectory["prior_alpha"],
        prior_weights=fixed_trajectory["prior_weights"],
        prior_means=fixed_trajectory["prior_means"],
        prior_factors=fixed_trajectory["prior_factors"],
    ...
    # Reload path.  The current architecture folder is the run directory for every new
    # diagnostic and decoder artefact; no new timestamped run folder is created.
    run_dir = Path.cwd().expanduser().resolve()
    conditioning_name = run_dir.name
    parent_run_dir = run_dir.parent
    (run_dir / "plots").mkdir(parents=True, exist_ok=True)
    (run_dir / "artefacts").mkdir(parents=True, exist_ok=True)
    print("Existing architecture directory:", run_dir)
    print("Main-model training is disabled; reloading saved model and diagnostics.")

    # Re-create the original evaluation set deterministically.  The original notebook did
    # not persist eval_data, but its seed and simulator are fixed by the saved config.
    eval_rng = np.random.default_rng(CFG.seed + 2_000)
    eval_data = simulate_mode_a_trajectories(
        eval_rng, CFG.n_eval_trajectories, CFG.trajectory_length, CFG
    )

    fixed_path = parent_run_dir / "artefacts" / "fixed_trajectory.npz"
    ...
    with np.load(fixed_path, allow_pickle=False) as fixed_npz:
        fixed_trajectory = {
            ...
            for key in (
                "theta_true", "observations", "num_sources", "theta_size",
                "prior_num_components", "prior_alpha", "prior_weights",
                "prior_means", "prior_factors",
            )
        }
...
#%% 21) Train the requested WM architecture(s), or reload the local best model
results: dict[str, dict[str, Any]] = {}
if train_wm:
    # Original training behaviour.
    ...
    # Local architecture-folder reload.  Recover the same result fields returned by
    # train_variant so every diagnostic below can run unchanged.
    conditioning_name = run_dir.name
    ...
    print("Reloading best model from:", model_path)
    best_model = load_model(
        model_path, CFG, conditioning_name, key=jax.random.key(0)  # type: ignore[arg-type]
    )
    ...
    # Re-plot the saved training diagnostics and best-model latent diagnostic locally.
    plot_training_diagnostics(
        history, best_epoch, conditioning_name, run_dir / "plots" / "training_diagnostics.png"
    )
    plot_latent_posterior_evolution(
        best_model, fixed_trajectory, fixed_prior_particles, CFG,
        run_dir / "plots" / "fixed_trajectory_best_model_latent.png",
        f"{conditioning_name}: best model (epoch {best_epoch}) in E-space",
    )
    results[conditioning_name] = {
        "model": best_model,
        "history": history,
        "best_epoch": best_epoch,
        "best_val_loss": best_val_loss,
...
def _make_fixed_decoder_training_set(
    ...
    *,
    ...
    # Use many independent meta-priors rather than one accidental mixture so the
    # post-hoc decoder sees the same broad theta support as the main theta embedder.
    decoder_chunks = []
    remaining = int(cfg.decoder_train_samples)
    while remaining > 0:
        chunk_size = min(max(cfg.decoder_batch_size, 1), remaining)
        decoder_chunks.append(
            sample_prior_np(
                rng,
                chunk_size,
                cfg,
    ...
    active = np.concatenate(decoder_chunks, axis=0)
    targets = (
...
def train_visualization_decoder(
    ...
    # Deliberately save to the standard local decoder path.  This overwrites any decoder
    # and decoder history previously stored for this architecture; no new run is created.
    decoder_path = variant_dir / "artefacts" / "visualization_decoder.eqx"
    history_path = variant_dir / "artefacts" / "visualization_decoder_history.npz"
    ...
    plot_visualization_decoder_training(
        history, conditioning, variant_dir / "plots" / "visualization_decoder_training.png"
    )
...
for name, result in results.items():
    ...
    decoder_path = variant_dir / "artefacts" / "visualization_decoder.eqx"
    decoder_history_path = variant_dir / "artefacts" / "visualization_decoder_history.npz"

    if train_decoder:
        print(
            f"[{name}] training a fresh visualisation decoder for {CFG.decoder_epochs} epochs; "
            f"the local decoder will be overwritten: {decoder_path}"
        )
        decoder, decoder_history = train_visualization_decoder(
            result["model"], name, variant_dir, CFG
        )
    else:
        if not decoder_path.is_file() or not decoder_history_path.is_file():
            raise FileNotFoundError(
                ...
                f"{variant_dir / 'artefacts'}"
            )
        print(f"[{name}] reloading visualisation decoder from: {decoder_path}")
        decoder = load_visualization_decoder(decoder_path, CFG, key=jax.random.key(0))
        ...
        plot_visualization_decoder_training(
            decoder_history, name, variant_dir / "plots" / "visualization_decoder_training.png"
        )
    ...
    plot_posterior_evolution(
        ...
        variant_dir / "plots" / "fixed_trajectory_best_model_decoded.png",
        f"{name}: decoded posterior evolution with the current post-hoc decoder",
    )
