...
def plot_reference_comparison(
    ...
    reference, ess = reference_posterior_particles_np(
        rng, observations, len(observations), S, theta_size, cfg, prior_spec=prior_spec
    )
...
def train_visualization_decoder(
    ...
    def plot_decoder_reconstruction_panel(epoch: int, epoch_metrics: dict[str, float]):
        ...
        observations = np.asarray(fixed_trajectory["observations"])
        ...
        if fixed_source_dim == 2:
            ...
            cloud_points.append(observations[:, :fixed_source_dim].reshape(-1, fixed_source_dim))
        ...
        for panel_index in range(sequence_nrows * sequence_ncols):
            ...
            if panel_index < decoded_post.shape[0] and fixed_source_dim == 2:
                ...
                designs_seen = observations[:t, :fixed_source_dim]
                ax.scatter(
                    designs_seen[:, 0],
                    designs_seen[:, 1],
                    marker="x",
                ...
                ax.grid(alpha=0.18)
                ax.set_title(f"decoded q_phi(theta | x_1:{t})", fontsize=10, fontweight="bold")
                if panel_index == 0:
