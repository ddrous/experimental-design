...
1. ObservationDimensionEmbedder: ONE padded [design coordinates, outcome] pair -> R^E.
2. ThetaDimensionEmbedder: padded source coordinates theta -> R^E.

There is no Likelihood Transformer.  At step t the observation embedder sees only the
current (x_t,y_t) pair, and that one E-vector is used as the AdaLN condition for the
Posterior Transformer.  The Posterior Transformer consumes the posterior point cloud
from step t-1 and transports it to the point cloud at step t.  The initial scan carry is
the embedded prior cloud.  The complete recurrence is implemented with jax.lax.scan, so
there is no Python loop over observation time.
...
observation_conditions [B, T, E]             one current-pair embedding per scan step
...
class BayesTransportConfig:
    ...
    # Sequential Posterior Transformer.  Each scan step receives only the current
    # embedded (design, outcome) pair through AdaLN and transports the current R^E cloud.
    hidden_dim: int = 256
    heads: int = 8
    mlp_ratio: int = 4
...
#%% 6) Sequential Posterior Transformer: AdaLN particle block
class AdaLNParticleBlock(eqx.Module):
    """Permutation-equivariant particle self-attention conditioned by one observation pair."""
    ...
    def __init__(
        self, hidden_dim: int, condition_dim: int, heads: int, mlp_dim: int, *, key: Array
    ):
        ...
        modulation = eqx.nn.Linear(condition_dim, 6 * hidden_dim, key=mod_key)
...
#%% 7) One-step embedding-space decoder and sequential AdaLN posterior update
...
class AdaLNPosteriorTransformer(eqx.Module):
    """One posterior update: current point cloud + current pair embedding -> next cloud."""
    ...
    def __call__(self, current_embeddings: Array, observation_embedding: Array) -> Array:
        particles = _linear_tokens(self.particle_in, current_embeddings)  # [N,H]
        for block in self.blocks:
            particles = block(particles, observation_embedding)
        return self.decoder(particles, current_embeddings)                # [N,E]


#%% 8) End-to-end dimension-agnostic sequential model
class ModeASequentialBayesModel(eqx.Module):
    """Observation-pair embedder + theta embedder + recurrent AdaLN Posterior Transformer."""
    ...
    def __init__(self, cfg: BayesTransportConfig, *, key: Array):
        observation_key, theta_key, posterior_key = jax.random.split(key, 3)
    ...
    def __call__(
        ...
        # The scan carry IS the current posterior point cloud.  At step t we embed exactly
        # one (design, outcome) pair, use only that vector as the AdaLN condition, transport
        # the current cloud, and pass the resulting posterior cloud to step t+1.
        def scan_step(current_embeddings: Array, observation_t: Array):
            observation_embedding = self.observation_embedder(
                observation_t, num_sources, theta_size
            )                                                            # [E]
            next_embeddings = self.posterior_transformer(
                current_embeddings, observation_embedding
            )                                                            # [N,E]
            return next_embeddings, (next_embeddings, observation_embedding)

        _, (posterior_sequence, observation_conditions) = jax.lax.scan(
            scan_step, prior_embeddings, observations
        )
        return posterior_sequence, observation_conditions, prior_embeddings  # [T,N,E], [T,E], [N,E]
...
def print_model_parameter_count(model: ModeASequentialBayesModel):
    ...
    total = observation_embedder + theta_embedder + posterior + sigreg
...
def plot_architecture_schematic(
    ...
    """Visual map of the sequential AdaLN architecture and fixed E interface."""
    ...
    ax.set_title("Sequential AdaLN posterior conditioning — recurrence compiled with jax.lax.scan",
                 loc="left", fontweight="bold")

    draw_box((0.01, 0.60), 0.14, 0.24, "[x_t,y_t] padded\nONE current pair", "scan input")
    draw_box((0.19, 0.58), 0.17, 0.28,
...
def plot_reference_comparison(
    ...
    column_names = ["sequential AdaLN", f"reference SNIS\nESS={ess:.0f}"]
...
def train_model(
    ...
    plot_latent_posterior_evolution(
        ...
        "sequential AdaLN: before training (identity transport in E-space)",
    )
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        save_json(
            ...
            {
                "training": "sequential posterior recurrence with jax.lax.scan",
                "conditioning": "AdaLN from one embedded design-outcome pair",
                "epoch": epoch,
        ...
        if epoch in visualisation_epochs:
            plot_latent_posterior_evolution(
                ...
                f"sequential AdaLN: latent posterior evolution after epoch {epoch}",
            )
    ...
    plot_latent_posterior_evolution(
        ...
        f"sequential AdaLN: best model (epoch {best_epoch}) in E-space",
    )
...
if train_wm:
    ...
    # architecture-specific subfolder because AdaLN is the only conditioning mechanism.
    run_dir = make_run_dir(CFG.env_name, CFG.runs_base)
    with (run_dir / "config.yaml").open("w", encoding="utf-8") as handle:
        yaml.safe_dump(asdict(CFG), handle, sort_keys=False)
    print("Run directory:", run_dir)

    # Precompute complete heterogeneous observation trajectories.  The neural training loop
    ...
    # the original saved configuration for fresh diagnostics without needing train_data.
    eval_rng = np.random.default_rng(CFG.seed + 2_000)
    eval_data = simulate_mode_a_trajectories(
        eval_rng, CFG.n_eval_trajectories, CFG.trajectory_length, CFG
    )
...
#%% 19) Train the sequential AdaLN model, or reload the local best model
...
def plot_visualization_decoder_training(
    ...
    ax.set_title("Post-hoc visualisation decoder — sequential AdaLN", fontweight="bold")
    ax.set_yscale("log")
    ax.grid(alpha=0.25)
    if destination is not None:
        fig.savefig(destination, dpi=170)
...
def train_visualization_decoder(
    ...
    for epoch in range(1, cfg.decoder_epochs + 1):
        ...
        if epoch == 1 or epoch % 100 == 0 or epoch == cfg.decoder_epochs:
            print(f"[sequential] visual decoder epoch {epoch:04d}: MSE={epoch_mse:.6f}")
...
plot_posterior_evolution(
    ...
    "sequential AdaLN: decoded posterior evolution with the current post-hoc decoder",
)
...
def structural_checks(
    ...
    2. Condition locality: the embedded conditions through t also ignore future observations.
...
#%% 27) Causal truncation consistency: full T versus running only the first t observations
...
summary = {
    ...
    "conditioning": "AdaLN from one embedded design-outcome pair per step",
