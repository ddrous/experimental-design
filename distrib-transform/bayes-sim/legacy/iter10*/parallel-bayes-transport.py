...
"""Dimension-agnostic single-cloud particle transport trained with an empirical energy score.
...
For each iid item, ONE N-particle reference cloud C=(z_1,...,z_N) is sampled iid from the
corresponding prior and is independent of (theta_i^*, Y_i).  The Posterior Transformer transports the
ENTIRE cloud jointly.  Its output defines the empirical posterior measure

    Q_hat_phi(C,Y) = N^{-1} sum_n delta_{T_phi(C,Y)_n}.

Training applies the exact empirical multivariate energy score ES(Q_hat_phi(C,Y), theta_i^*): every
particle is attracted to the single theta_i^* that generated Y_i, while the pairwise term rewards
spread of the transported empirical measure.  Particle self-attention does not violate this scoring
interpretation because the forecast being scored is the whole empirical measure, not N independent
scalar transport outputs.
...
prior_particles         [B,N,Smax,Dmax]  # one iid reference cloud
...
if train_wm:
    ...
    # evaluation_trajectory_length is runtime-only: when reloading an old model we deliberately
    # take the CURRENT script value so the same trained model can be tested on a longer rollout.
    _saved_cfg_dict["evaluation_trajectory_length"] = _script_cfg.evaluation_trajectory_length
...
def sample_prior_law_and_truth_np(
    rng: np.random.Generator,
    cfg: BayesTransportConfig,
    *,
    num_sources: int,
    ...
    """Draw the task prior first, then draw the observation-generating theta from that prior.

    This is the single simulator path for creating a truth parameter.  With a meta-prior,
    the sampling order is pi ~ H followed by theta* ~ pi; with the fixed-prior setting it
    reduces to theta* ~ N(0, prior_std^2 I).  Reference particle clouds are sampled later,
    independently, from the same stored `prior_spec`.
    """
    prior_spec = sample_prior_spec_np(rng, cfg, source_dim=int(source_dim))
    theta_true = sample_prior_np(
        rng,
        1,
        cfg,
...
def simulate_trajectories(
    ...
    *,
    ...
    for m in range(n_trajectories):
        ...
        prior_spec, theta_active = sample_prior_law_and_truth_np(
            rng, cfg, num_sources=S, source_dim=D
        )
        designs = rng.uniform(
            cfg.design_low,
            cfg.design_high,
...
def make_iid_batch_np(
    ...
    *,
    ...
    """Create one iid minibatch with ONE iid reference cloud per joint draw.

    Data Assumption 2.5 concerns the iid joint rows (theta_b^*, Y_b).  For each row we draw
    one independent cloud C_b=(z_1,...,z_N) with z_n iid from the matching prior.  The whole
    cloud is then transported jointly and scored as an empirical predictive distribution.

    In the default fixed-rho setting, C_b is independent of (theta_b^*, Y_b), exactly as
    required for Monte Carlo evaluation of the expected energy-score objective.  If
    `use_meta_prior=True`, the same construction is applied conditionally on the sampled
    task prior pi_b: theta_b^* and C_b are independent draws from pi_b.
    """
...
class ContinuousJointDataset(IterableDataset):
    """Infinite stream of fresh iid joint samples and one independent prior cloud.
...
class FixedShapeObservationEmbedder(eqx.Module):
    """Direct observation projection used when the problem shape is fixed.

    When both S and D are fixed there is no need to learn a dimension-aggregation
    operator.  We therefore normalize the full fixed-size (design, outcome) vector
    and project it once to the shared embedding width.  No dimension Transformer,
    scalar-token encoder, positional pool, or padding mask is instantiated.
    """
...
class FixedShapeThetaEmbedder(eqx.Module):
    """Direct theta projection used when the problem shape is fixed.

    The fixed active [S,D] block is flattened once and projected to the shared
    embedding width.  This deliberately avoids the dimension-agnostic scalar-token
    Transformer while preserving the existing normalization and optional source
    ...
    def __init__(self, cfg: BayesTransportConfig, *, key: Array):
        ...
        self.projection = eqx.nn.Linear(
            self.num_sources * self.source_dim,
            cfg.embedding_dim,
            key=key,
        )
    ...
    def __call__(self, theta: Array, num_sources: Array, theta_size: Array) -> Array:
        ...
        values = theta[: self.num_sources, : self.source_dim].reshape(-1) / self.prior_std
...
def batch_objective(
    ...
    Every batch row is one iid joint draw (theta*, Y) from Data Assumption 2.5 and carries
    ONE independent iid prior cloud C.  The causal observation encoder is evaluated once, and
    the direct Posterior Transformer is vmapped over every o=Omin,...,Omax.  Crucially, every
    prefix starts from that SAME original prior cloud; no posterior output is fed into another
    training prefix.

    For a fixed row and prefix the model output is an empirical predictive measure Q_hat.
    We optimize ES(Q_hat, theta*) directly.  Conditional on the observations Y=y and reference
    cloud C, theta* has posterior law pi(.|y), so the energy-distance identity gives

        2 E[ES(Q_hat, theta*) | y,C]
          = D_E^2(pi(.|y), Q_hat) + const(y).

    Therefore averaging this score over iid joint rows, independent reference clouds, and all
    prefixes is a valid Monte Carlo empirical-risk objective for posterior approximation.  No
    independence assumption on transported particles is required because Q_hat itself is the
    reported distribution being scored.
    """
...
def plot_architecture_schematic(
    ...
    draw_box(
        (0.02, 0.49), 0.14, 0.12,
        "one iid N-particle cloud\nindependent of (theta*, Y)",
        "reference cloud C",
    )
    ...
    fig.suptitle(
        "Amortized empirical-energy transport and repeated-Bayes evaluation",
...
def train_model(
    ...
    prefix counts Omin,...,Omax.  Every prefix starts from the same original independent prior
    cloud.  The optimized loss is the exact empirical multivariate energy score of that
    transported cloud, averaged over batch rows and prefixes.
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        save_json(
            run_dir / "artefacts" / "training_state.json",
            {
                ...
                "reference_clouds_per_joint_draw": 1,
...
if train_wm:
    ...
    # than truncating to the trajectory length stored when the model was trained.  The same
    # deterministic simulator seed is used, and the simulator itself draws pi first and then
    # theta* ~ pi, so meta-prior provenance remains correct on reload as well.
    fixed_rng = np.random.default_rng(CFG.seed + 2_500)
    fixed_data = simulate_trajectories(
        fixed_rng,
        1,
...
summary = {
    ...
    "reference_clouds_per_training_joint_draw": 1,
    "reference_cloud_independent_of_joint_target": True,
