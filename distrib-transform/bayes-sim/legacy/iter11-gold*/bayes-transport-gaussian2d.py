...
experiment. It intentionally preserves the main training and evaluation structure:
  ...
  * synthetic particle-wise interpolated input clouds with one tau per particle,
...
The exact Gaussian posterior remains the sequential sanity-check reference.  The synthetic
particle-wise training priors are generally non-Gaussian mixtures, so their train-time moment
diagnostic instead Bayes-reweights the actual finite input cloud. In particular:
  * every training step records learned-vs-empirical-Bayes posterior moment errors for the actual input cloud;
  * sequential evaluation records learned-vs-exact-Gaussian errors at every repeated-Bayes step;
...
For each fresh iid training row:
  1. Draw one interpolation anchor a ~ N(mu_0,Sigma_0).  The anchor is deliberately NOT the
     supervised truth; using the truth itself as the anchor would leak the answer into the input cloud.
  2. Draw one cloud-level interpolation mean m and then independent particle-wise coefficients
     tau_n.  By default tau_n | m ~ Beta(kappa m, kappa (1-m)); ``iid_uniform`` is also available.
  3. Draw z_n ~ N(mu_0,Sigma_0) independently and form
         c_n = (1-tau_n) z_n + tau_n a.
     Conditional on the anchor and tau law, the c_n are iid from a one-dimensional Gaussian
     location/scale mixture.  Its first two moments are available in closed form.
  4. Draw an independent tau_* from the SAME tau law and z_* ~ N(mu_0,Sigma_0), then set
         theta* = (1-tau_*) z_* + tau_* a.
     This is exact ancestral sampling from the same population law as every input particle; no
     empirical resampling approximation is needed.
  5. Draw one y ~ N(A theta*, R), transport the cloud, and optimize the same empirical energy score.

The default hierarchical-Beta construction is a small refinement of iid tau_n ~ Uniform[0,1].
Pure iid-uniform taus make almost every large-N cloud have the same mixture of interpolation
strengths. Sampling a cloud-level mean m first preserves cloud-to-cloud concentration diversity
while still giving particle-level variation. kappa -> infinity recovers the old shared-tau
construction, whereas smaller kappa creates more heterogeneous particles.
...
prior_mean        [B,K]       exact first moment of the synthetic mixture input law
prior_cov         [B,K,K]     exact covariance of the synthetic mixture input law
...
class BayesTransportConfig:
    ...
    # Synthetic training-prior interpolation. Priors are generated WITHOUT using the likelihood.
    # ``hierarchical_beta`` (default): draw a cloud-level mean m, then one tau_n per particle
    # from Beta(kappa*m, kappa*(1-m)). ``iid_uniform`` implements tau_n iid Uniform[0,1].
...
def _sample_particlewise_taus_np(
    rng: np.random.Generator, n: int, cfg: BayesTransportConfig = CFG
) -> tuple[np.ndarray, float, float, float]:
    """Draw row-specific particle-wise interpolation coefficients and their exact moments.

    Returns ``taus, E[tau], Var(tau), cloud_tau_mean``.  The last quantity is useful for
    interpretation; for ``iid_uniform`` it is exactly 1/2.
    """
    mode = cfg.interpolation_tau_mode.strip().lower()
    n = int(n)

    if mode == "iid_uniform":
        taus = rng.uniform(0.0, 1.0, size=n)
        return taus.astype(np.float32), 0.5, 1.0 / 12.0, 0.5

    if mode == "hierarchical_beta":
        low = float(cfg.interpolation_tau_mean_low)
        high = float(cfg.interpolation_tau_mean_high)
        kappa = float(cfg.interpolation_tau_concentration)
        if not (0.0 < low < high < 1.0):
            raise ValueError("interpolation_tau_mean_low/high must satisfy 0 < low < high < 1.")
        if kappa <= 0.0:
            raise ValueError("interpolation_tau_concentration must be positive.")

        cloud_tau_mean = float(rng.uniform(low, high))
        alpha = kappa * cloud_tau_mean
        beta = kappa * (1.0 - cloud_tau_mean)
        taus = rng.beta(alpha, beta, size=n)
        tau_mean = alpha / (alpha + beta)
        tau_var = alpha * beta / ((alpha + beta) ** 2 * (alpha + beta + 1.0))
        return taus.astype(np.float32), float(tau_mean), float(tau_var), cloud_tau_mean

    raise ValueError(
        f"Unknown interpolation_tau_mode={cfg.interpolation_tau_mode!r}; "
        "choose 'hierarchical_beta' or 'iid_uniform'."
    )


def particlewise_interpolation_moments_np(
    anchor: np.ndarray, tau_mean: float, tau_var: float, cfg: BayesTransportConfig = CFG
) -> tuple[np.ndarray, np.ndarray]:
    r"""Exact first two moments of one particle in the particle-wise interpolation law.

    Let ``Z ~ N(mu_0,Sigma_0)``, T independent of Z, delta = anchor-mu_0, and

        C = (1-T) Z + T anchor.

    Then

        E[C | anchor] = mu_0 + E[T] delta,

        Cov[C | anchor]
          = E[(1-T)^2] Sigma_0 + Var(T) delta delta^T
          = ((1-E[T])^2 + Var(T)) Sigma_0 + Var(T) delta delta^T.

    For T~Uniform[0,1], E[T]=1/2 and Var(T)=1/12, hence

        E[C|anchor]   = (mu_0+anchor)/2,
        Cov[C|anchor] = Sigma_0/3 + (anchor-mu_0)(anchor-mu_0)^T/12.

    For T|m ~ Beta(kappa*m,kappa*(1-m)),

        E[T|m]=m,  Var(T|m)=m(1-m)/(kappa+1).

    The full law is generally a non-Gaussian one-dimensional Gaussian location/scale mixture,
    but exact ancestral sampling is still trivial.
    """
    mu0 = base_prior_mean_np(cfg)
    Sigma0 = base_prior_cov_np(cfg)
    anchor = np.asarray(anchor, dtype=np.float64).reshape(K)
    delta = anchor - mu0
    second_one_minus = (1.0 - float(tau_mean)) ** 2 + float(tau_var)
    mean = mu0 + float(tau_mean) * delta
    cov = second_one_minus * Sigma0 + float(tau_var) * np.outer(delta, delta)
    cov = 0.5 * (cov + cov.T)
    return mean.astype(np.float32), cov.astype(np.float32)
...
def sample_interpolated_prior_and_truth_np(
    ...
    r"""Sample a particle-wise interpolated cloud and an iid truth from its exact population law.

    A shared anchor a ~ N(mu_0,Sigma_0) defines the row-level mixture. Conditional on the anchor
    and row-specific tau distribution, every input particle is generated independently as

        C = (1-T) Z + T a,       Z ~ N(mu_0,Sigma_0).

    theta* uses a fresh independent (T_*, Z_*) from the SAME law. Thus theta* and all input
    particles are iid conditional on the row-level latent variables. The anchor is not theta*.
    """
    n_particles = int(n_particles)
    anchor = sample_base_prior_np(rng, 1, cfg)[0].astype(np.float64)

    # N particle taus + one independent tau_* for theta*.
    taus_all, tau_mean, tau_var, _cloud_tau_mean = _sample_particlewise_taus_np(
        rng, n_particles + 1, cfg
    )
    base_all = sample_base_prior_np(rng, n_particles + 1, cfg).astype(np.float64)
    tau_col = np.asarray(taus_all, dtype=np.float64)[:, None]
    interpolated_all = (1.0 - tau_col) * base_all + tau_col * anchor[None, :]

    prior_particles = interpolated_all[:n_particles].astype(np.float32)
    theta_true = interpolated_all[n_particles].astype(np.float32)
    prior_mean, prior_cov = particlewise_interpolation_moments_np(anchor, tau_mean, tau_var, cfg)
...
def simulate_iid_joint_samples(
    ...
    """Fixed iid particle-wise-mixture (C, theta*, y) rows used for amortized validation."""
...
class ContinuousJointDataset(IterableDataset):
    """Infinite CPU stream of fresh iid particle-wise-mixture (C, theta*, y) training rows."""
...
def empirical_prior_update_moments_single(
    prior_particles: Array, observation: Array
) -> tuple[Array, Array]:
    """Bayes-update the ACTUAL finite input cloud for train-time diagnostics only.

    Particle-wise interpolation gives a Gaussian-mixture population prior, so the usual Gaussian
    mean/covariance update is not exact. We instead reweight the empirical prior particles by the
    known likelihood and compare the learned cloud with those weighted moments. The likelihood is
    not used to generate the prior and this reference never enters the training loss.
    """
    predicted_y = observation_mean_jax(prior_particles, CFG)
    residual = (observation[None, :] - predicted_y) / CFG.observation_noise_std
    log_weights = -0.5 * jnp.sum(residual**2, axis=-1)
    weights = jax.nn.softmax(log_weights)
    mean = jnp.sum(weights[:, None] * prior_particles, axis=0)
    centered = prior_particles - mean[None, :]
    cov = jnp.einsum("n,ni,nj->ij", weights, centered, centered)
    cov = 0.5 * (cov + cov.T)
    return mean, cov
...
def batch_objective(model: SequentialBayesModel, batch: dict[str, Array]) -> tuple[Array, dict[str, Array]]:
    """Non-sequential iid energy-score objective plus an empirical-prior Bayes diagnostic.

    theta_true is still an exact independent draw from the same synthetic population law as the
    input particles, so the proper-scoring-rule training objective is unchanged.
    """
    ...
    reference_mean, reference_cov = jax.vmap(empirical_prior_update_moments_single)(
        batch["prior_particles"], batch["observation"]
    )
    reference_mean_error = jax.vmap(exact_mean_rmse_single)(predicted, reference_mean)
    reference_cov_error = jax.vmap(exact_cov_frobenius_single)(predicted, reference_cov)
    ...
    metrics = {
        ...
        "reference_mean_rmse": jnp.mean(reference_mean_error),
        "reference_cov_frobenius": jnp.mean(reference_cov_error),
    }
    return loss, metrics
...
def evaluate_amortized_model(
    ...
    *,
    ...
    names = [
        ...
        "repulsion",
        "reference_mean_rmse",
        "reference_cov_frobenius",
    ]
    values = {name: [] for name in names}
    weights = []
    batch_fields = ("theta_true", "observation", "prior_particles")
...
def plot_posterior_evolution(
    ...
    truth_color = "#111111"
    seen_location_color = "#A8DDB5"  # light green x: location used to generate observations already seen
    ...
    for panel_index, (ax, cloud, exact_mean, exact_cov, label) in enumerate(
        ...
        if panel_index > 0:
            # All y_t in this sanity check are generated from the same latent theta*. Draw the
            # requested light-green x behind the truth marker to show where the observations
            # already seen in this panel were computed.
            t_seen = steps[panel_index - 1]
            # ax.scatter(
            #     theta_true[0], theta_true[1], marker="x", s=255, color=seen_location_color,
            #     linewidths=2.8, alpha=0.95, zorder=5,
            #     label=rf"location generating seen $y_{{1:{t_seen}}}$",
            # )
        ax.scatter(
            theta_true[0], theta_true[1], marker="*", s=170, color=truth_color,
            edgecolors="white", linewidths=0.7, zorder=7, label=r"$\theta^\star$",
        )
        if panel_index > 0:
...
def plot_training_diagnostics(
    ...
    axes[0, 3].plot(
        train_steps, np.maximum(history["step_reference_mean_rmse"], 1e-12), linewidth=0.82,
        label=r"RMSE$(\widehat\mu_\phi,\mu_{\rm emp})$",
    )
    axes[0, 3].plot(
        train_steps, np.maximum(history["step_reference_cov_frobenius"], 1e-12), linewidth=0.82,
        label=r"$\|\widehat\Sigma_\phi-\Sigma_{\rm emp}\|_F$",
    )
    axes[0, 3].set_title(r"Empirical-prior Bayes reference at every train step", loc="left", fontweight="bold")
    ...
    fig.suptitle(
        r"2-D linear-Gaussian transport: particle-wise synthetic priors + exact sequential diagnostics",
        fontsize=14, fontweight="bold",
    )
    if destination is not None:
        fig.savefig(destination, dpi=180, bbox_inches="tight")
...
def train_model(
    ...
    history: dict[str, list] = {
        ...
        "step_repulsion": [],
        "step_grad_norm": [],
        "step_reference_mean_rmse": [],
        "step_reference_cov_frobenius": [],
        "epoch_train_loss": [],
        "epoch_learning_rate": [],
        "epoch_val_loss": [],
        "epoch_val_mean_rmse": [],
        "epoch_val_reference_mean_rmse": [],
        "epoch_val_reference_cov_frobenius": [],
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for _ in progress:
            ...
            history["step_reference_mean_rmse"].append(float(host["reference_mean_rmse"]))
            history["step_reference_cov_frobenius"].append(float(host["reference_cov_frobenius"]))
            progress.set_postfix(
                ES=f"{host_loss:.4f}",
                ref_mu=f"{float(host['reference_mean_rmse']):.4f}",
                ref_cov=f"{float(host['reference_cov_frobenius']):.4f}",
                refresh=False,
            )
        ...
        history["epoch_val_reference_mean_rmse"].append(float(val_metrics["reference_mean_rmse"]))
        history["epoch_val_reference_cov_frobenius"].append(float(val_metrics["reference_cov_frobenius"]))
        ...
        print(
            ...
            f"val empirical-ref mean RMSE={float(val_metrics['reference_mean_rmse']):.5f} | "
            f"val empirical-ref cov F={float(val_metrics['reference_cov_frobenius']):.5f} | "
...
print(
    "Synthetic interpolation:", CFG.interpolation_tau_mode,
    f"(kappa={CFG.interpolation_tau_concentration:g}, "
    f"mean range=[{CFG.interpolation_tau_mean_low:g}, {CFG.interpolation_tau_mean_high:g}])",
)
...
summary = {
    ...
    "training_mode": "non-sequential amortized transport from particle-wise interpolated mixture input clouds",
    ...
    "synthetic_truth_sampling_mode": "exact ancestral draw from the same particle-wise interpolation mixture",
