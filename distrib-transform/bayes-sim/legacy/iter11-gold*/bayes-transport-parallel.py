...
def sample_interpolated_training_prior_and_truth_np(
    ...
    *,
    ...
    # # Logit-normal tau: sample z ~ N(m, s^2), then squash through the sigmoid
    # m, s = -1.5, 1.0  # tune these to control where tau concentrates
    # logit_tau = rng.normal(loc=m, scale=s)
    # tau = np.float32(1.0 / (1.0 + np.exp(-logit_tau)))

    # a, b = 1.0, 5.0  # tune these to control where tau concentrates
    # tau = np.float32(rng.beta(a, b))
    ...
    if mode == "closed_form":
        ...
        ## Take a noisy approximation of the mean of the cloud, which is a better representative of the underlying distribution than a single particle.
        theta_true = np.mean(prior_particles, axis=0) + rng.normal(0, 0.30, size=(S, D)).astype(np.float32)
