...
observations enter the causal likelihood Transformer from physical D+1 width, with zero learnable
parameters in either fixed-shape input embedder.  The likelihood Transformer itself owns a learned
input projection from that physical observation width into its separately configured hidden width.
...
3. Draw the actual observation-generating theta* according to `synthetic_truth_sampling_mode`:
   * "closed_form" (DEFAULT): sample theta* independently from the exact conditional law represented
     by C_tau given (theta_tilde,tau).  For a Gaussian rho_0 this is
       N(tau*theta_tilde, ((1-tau)*prior_std)^2 I); for a Uniform rho_0 it is Uniform on the
       coordinatewise box [(1-tau)*design_low + tau*theta_tilde,
                           (1-tau)*design_high + tau*theta_tilde].
     Thus, conditional on (theta_tilde,tau), theta* and the N cloud particles are iid from the same
     interpolated prior law.
   * "empirical": sample theta* uniformly from the N realised particles in C_tau, i.e. directly from
     the empirical prior measure supplied to the network.
   * "bernoulli_choice": preserve the previous two-path experiment: with probability
   ...
   Only the resulting theta* belongs to the simulator datum; theta_tilde and tau remain synthetic
   dataloader variables used to define the input prior.
...
compatible with this score.  In the default closed-form mode, conditional on the latent
(theta_tilde,tau), theta* and the input particles are independent draws from the SAME interpolated
prior law.  The empirical mode instead draws theta* from the realised empirical cloud, while the
legacy Bernoulli mode retains the previous matched/misspecified construction.
...
zero learnable parameters.  Independently of that choice, the likelihood Transformer projects its input
into its own configured hidden width L before applying causal self-attention.  In both regimes the output
...
L : likelihood Transformer hidden/context width
...
observation_contexts    [B,Omax,L] after the likelihood Transformer's own input projection
...
class BayesTransportConfig:
    ...
    prior_std: float = 1.0
    # How the observation-generating theta* is drawn once C_tau has been constructed:
    #   "closed_form"      : DEFAULT; fresh draw from the exact interpolated prior law represented by C_tau.
    #   "empirical"        : draw uniformly from the realised N-particle empirical prior C_tau.
    ...
    max_observations_per_step: int = 2
    ...
    # Causal likelihood Transformer.  This has its OWN learned input projection and hidden
    # width, exactly as the Posterior Transformer projects physical/theta embeddings into
    # `hidden_dim`.  In fixed-shape mode the projection is (D+1) -> likelihood_hidden_dim;
    # in heterogeneous mode it is embedding_dim -> likelihood_hidden_dim.  Its output tokens
    # have width likelihood_hidden_dim and condition every Posterior Transformer prefix.
...
def sample_interpolated_training_prior_and_truth_np(
    ...
    *,
    ...
    The CPU sampling order is exactly the experiment described at the top of this file:
      ...
      3. choose theta* according to `synthetic_truth_sampling_mode`.

    In the DEFAULT `closed_form` mode, theta* is a fresh independent draw from the exact
    conditional distribution represented by one C_tau particle given (theta_tilde,tau):

      Gaussian rho_0 = N(0, prior_std^2 I):
          theta* | theta_tilde,tau ~ N(
              tau*theta_tilde, ((1-tau)*prior_std)^2 I
          ).

      Uniform rho_0 = Uniform([design_low,design_high]^(S*D)):
          theta* | theta_tilde,tau is coordinatewise Uniform on
          [(1-tau)*design_low  + tau*theta_tilde,
           (1-tau)*design_high + tau*theta_tilde].

    `empirical` instead samples one realised full-theta particle uniformly from C_tau.
    `bernoulli_choice` preserves the previous p-matched / (1-p)-mismatched construction.
    ...
    prior_particles = (
        one_minus_tau * base_particles
        + tau * potential_theta[None, :, :]
    ).astype(np.float32)

    mode = cfg.synthetic_truth_sampling_mode
    if mode == "closed_form":
        # Sample from the exact population law whose iid Monte Carlo realisation is C_tau,
        # rather than selecting one of the finite particles already shown to the network.
        if cfg.base_prior_distribution == "gaussian":
            mean = tau * potential_theta
            std = float(one_minus_tau) * float(cfg.prior_std)
            theta_true = rng.normal(
                loc=np.asarray(mean, dtype=np.float64),
                scale=std,
                size=(S, D),
            ).astype(np.float32)
        elif cfg.base_prior_distribution == "uniform":
            ...
            # `tau` is sampled in [0,1), but float32 rounding can in principle make the
            # interval degenerate.  NumPy supports equal bounds; this explicit branch makes
            # the intended point-mass limit at tau=1 unambiguous.
            if np.all(high == low):
        ...
        # A draw from the exact empirical prior measure represented by the input cloud.
...
def sample_interpolated_prior_given_truth_np(
    ...
    *,
    ...
    an independent base-prior draw.  Fresh z_n and tau ~ Uniform[0,1] then build the cloud.
...
class LikelihoodSequenceEmbedder(eqx.Module):
    """Project and causally contextualise all Omax observation tokens exactly once.
    ...
    The dimension-aggregation module and the likelihood Transformer have deliberately separate
    responsibilities.  In heterogeneous-shape mode the input width is the shared aggregation
    width E; in fixed-shape mode it is the physical observation width D+1.  In BOTH cases this
    Transformer owns a learned input projection into `likelihood_hidden_dim`, then applies its
    independently configured causal blocks.  Thus bypassing the fixed-shape observation embedder
    does not force the likelihood Transformer itself to operate at the tiny physical width.
    """
...
class SequentialBayesModel(eqx.Module):
    ...
    def __init__(self, cfg: BayesTransportConfig, *, key: Array):
        ...
        if self.fixed_shape:
            ...
            # Posterior Transformer from width S*D, while observations enter the likelihood
            # Transformer's OWN learned input projection directly from physical width D+1.
        ...
        self.observation_context_dim = cfg.likelihood_hidden_dim
    ...
    def _encode_observation_block(
        ...
        return self.likelihood_embedder(pair_embeddings)  # [Omax,likelihood_hidden_dim] in both regimes
...
def print_model_parameter_count(model: SequentialBayesModel):
    ...
    if model.fixed_shape:
        print(
            ...
            f"likelihood hidden/context={model.observation_context_dim}"
        )
